...
export async function GET(request) {
  try {
    ...
    console.log(
      "🔍 API Depósitos - Obteniendo depósitos para restaurante:",
      restauranteId
    );

    // Obtener todos los documentos de la colección Depositos
    const depositosRef = collection(
      db,
      "restaurantes",
      restauranteId,
      "Depositos"
    ...
    try {
      ...
      console.log(
        "ℹ️ Colección Depositos no existe aún, retornando array vacío"
      );
      return NextResponse.json({
        success: true,
        data: [],
    ...
    depositosSnapshot.forEach((doc) => {
      const data = doc.data();
      if (
        data.historialDepositos &&
        typeof data.historialDepositos === "object"
      ) {
        depositos.push({
          id: doc.id,
    ...
    console.log("✅ Depósitos obtenidos exitosamente:", {
      ...
      totalDepositos: depositos.reduce((sum, deposito) => {
        if (
          deposito.historialDepositos &&
          typeof deposito.historialDepositos === "object"
        ) {
          const depositoTotal = Object.values(
            deposito.historialDepositos
          ).reduce((depositoSum, pago) => {
