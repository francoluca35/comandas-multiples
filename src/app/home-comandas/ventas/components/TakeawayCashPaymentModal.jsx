...
function TakeawayCashPaymentModal({ isOpen, onClose, orderData, onPaymentComplete }) {
  ...
  const registrarIngresoAutomatico = async (monto, cliente, opcionPago = "caja") => {
    try {
      ...
      const tipoIngreso = "Venta Takeaway";
  ...
  const enviarACocina = async () => {
    try {
      console.log("🚀 Enviando pedido takeaway a cocina...");
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-lg p-6 w-full max-w-md mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-white">Pago en Efectivo - Takeaway</h2>
          <button
            onClick={onClose}
      ...
      <CashAccountSelectionModal
        ...
        tipoVenta="Venta Takeaway"
      />
    </div>
  );
}

export default TakeawayCashPaymentModal;
