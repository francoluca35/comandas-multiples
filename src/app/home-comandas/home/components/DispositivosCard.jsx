...
import VerMensajesModal from "./VerMensajesModal";
...
function DispositivosCard() {
  ...
  const { dispositivos, usuariosActivos, loading, obtenerEstadisticas, obtenerMensajesUsuario } =
    useDispositivos();
  const { usuario } = useAuth();
  const { isAdmin } = useRole();

  // Verificar si el usuario es admin
  const esAdmin = isAdmin || usuario?.rol === "admin" || usuario?.esAdmin;

  // Verificar mensajes pendientes cuando no es admin
  const verificarMensajes = useCallback(async () => {
    try {
      const usuarioId = localStorage.getItem("usuarioId") || usuario?.id;
      if (usuarioId) {
        const mensajes = await obtenerMensajesUsuario(usuarioId);
        setMensajesPendientes(mensajes || []);
        setTieneMensajes((mensajes || []).length > 0);
      }
    } catch (error) {
      console.error("Error verificando mensajes:", error);
      setTieneMensajes(false);
    }
  }, [usuario?.id, obtenerMensajesUsuario]);
  ...
  }, [esAdmin, usuario?.id, verificarMensajes]);
    ...
    <>
      <div className="bg-[#2a2a2a] rounded-lg p-4 sm:p-6 text-white h-full">
          ...
          <div className="mb-4">
            ...
            <div className="space-y-2 max-h-24 overflow-y-auto">
              {dispositivos.slice(0, 3).map((dispositivo) => (
                <div
                  key={dispositivo.id}
                  className="flex items-center justify-between bg-[#3a3a3a] rounded px-2 py-1"
                >
        ...
        <div className="space-y-2">
          ...
          {/* Botón de ver usuarios (solo admin) o ver mensajes (no admin) */}
          {esAdmin ? (
            <button
            ...
            <button
              onClick={() => {
                setShowMensajesModal(true);
                verificarMensajes(); // Actualizar mensajes al abrir
              }}
              disabled={!tieneMensajes}
              className={`w-full rounded-lg px-4 py-2 text-sm font-medium flex items-center justify-center transition-all duration-200 ${
                tieneMensajes
                  ? "bg-blue-600 hover:bg-blue-700 text-white"
                  : "bg-gray-600 text-gray-400 cursor-not-allowed"
            ...
            >
              ...
              Ver Mensajes
              {tieneMensajes && mensajesPendientes.length > 0 && (
                <span className="ml-2 bg-red-500 text-white text-xs rounded-full px-2 py-0.5">
                  {mensajesPendientes.length}
                </span>
              )}
            </button>
          )}
      ...
      {/* Modal de ver usuarios (solo admin) */}
      {esAdmin && (
        <VerUsuariosModal
      ...
      {/* Modal de ver mensajes (no admin) */}
      {!esAdmin && (
        <VerMensajesModal
          isOpen={showMensajesModal}
          onClose={() => {
            setShowMensajesModal(false);
            verificarMensajes(); // Actualizar mensajes al cerrar
          }}
        />
      )}
    </>
