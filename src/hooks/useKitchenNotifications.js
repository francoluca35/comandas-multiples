...
const useKitchenNotifications = () => {
  ...
  const notifyOrderReady = useCallback((orderData, playSound = true) => {
    const { mesa, cliente, productos, tipo } = orderData;
    ...
    // Mostrar notificación visual (sin sonido si playSound es false)
    if (playSound) {
      showNotification(message, "order-ready");
    } else {
      // Solo mostrar notificación visual sin sonido
      const notification = {
        id: Date.now(),
        message,
        type: "order-ready",
      ...
      console.log("🔔 Notificación visual mostrada (sin sonido):", notification);
    }
    
    // Vibrar dispositivo solo si playSound es true
    if (playSound) {
      vibrateDevice();
    }
