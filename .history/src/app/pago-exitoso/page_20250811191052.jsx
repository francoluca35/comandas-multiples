    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-md">
        <div className="text-center">
          ...
          {/* Estado */}
          <p className={`text-xl font-semibold ${getStatusColor(paymentStatus?.status || status)} mb-6`}>
            ...
            <div className="bg-gray-50 rounded-lg p-6 mb-6">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-semibold ${getStatusColor(paymentStatus.status)}`}>
                    {getStatusText(paymentStatus.status)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Método:</span>
                  <span className="font-semibold">{paymentStatus.payment_method?.type || "Tarjeta"}</span>
                </div>
                {paymentStatus.transaction_amount && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Monto:</span>
                    <span className="font-semibold">${paymentStatus.transaction_amount.toLocaleString()}</span>
                  </div>
                )}
          ...
          {paymentStatus?.status === "pending" && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              <p className="text-yellow-800 font-medium">
                El pago está siendo procesado. Por favor, espera la confirmación.
              </p>
            </div>
          )}
