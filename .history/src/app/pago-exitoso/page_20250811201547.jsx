...
export default function PagoExitosoPage() {
    ...
    if (paymentId) {
      checkPaymentStatus(paymentId)
        .then((payment) => {
          ...
          // Si el pago fue aprobado, intentar imprimir el ticket automáticamente
          if (payment.status === "approved") {
            // Extraer datos del pedido desde la información adicional del pago
            const orderDataFromPayment = {
              mesa: payment.restaurant_name || "Mesa",
              productos: [
                {
                  producto: "Pedido",
                  unidades: 1,
            ...
            setOrderData(orderDataFromPayment);
            
            // Imprimir ticket automáticamente
            printTicket(orderDataFromPayment)
              .then(() => {
                console.log("Ticket impreso automáticamente");
              })
              .catch((error) => {
                console.error("Error al imprimir ticket automáticamente:", error);
              });
          }
  ...
  }, [paymentId, checkPaymentStatus, printTicket]);
                  ...
                  onClick={() => {
                    if (orderData) {
                      printTicket(orderData)
                        .then(() => console.log("Ticket impreso manualmente"))
                        .catch((error) => console.error("Error al imprimir ticket:", error));
                    }
                  }}
