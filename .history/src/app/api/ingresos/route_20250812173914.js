...
async function actualizarCajaRegistradoraIngreso(
  ...
  try {
    ...
    if (cajaSnapshot.empty) {
      console.log(
        "📦 No hay cajas registradoras disponibles, creando una nueva..."
      );
      ...
      console.log("✅ Nueva caja registradora creada con ID:", nuevaCaja.id);
      return;
    }
...
async function actualizarDineroVirtualIngreso(
  ...
  try {
    ...
    if (dineroSnapshot.empty) {
      console.log("💳 No hay documentos de dinero virtual disponibles, creando uno nuevo...");
      // Crear un nuevo documento de dinero virtual
      const nuevoDinero = await addDoc(dineroRef, {
        Virtual: monto.toString(),
        IngresosVirtual: {
          [new Date().toISOString()]: {
            fecha: new Date(fecha),
            monto: monto.toString(),
      ...
      console.log("✅ Nuevo documento de dinero virtual creado con ID:", nuevoDinero.id);
      return;
    }
