  ...
  try {
    ...
    if (!mainCategoryId || !name || name.trim() === "") {
      return NextResponse.json(
        {
          success: false,
          error: "ID de categoría principal y nombre de subcategoría son obligatorios",
        },
        { status: 400 }
      );
    }
...
export async function PUT(request) {
  try {
    const body = await request.json();
    const { mainCategoryId, oldName, newName, restaurantId = "francomputer" } = body;
    ...
    if (!mainCategoryId || !oldName || !newName || newName.trim() === "") {
      return NextResponse.json(
        {
          success: false,
          error: "ID de categoría principal, nombre anterior y nuevo nombre son obligatorios",
        },
        { status: 400 }
      );
    }
