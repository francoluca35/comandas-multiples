  ...
  try {
    ...
    stockSnapshot.forEach((doc) => {
      ...
      if (!isNaN(costo) && !isNaN(stock) && costo > 0 && stock > 0) {
        ...
        productos.push({
          id: doc.id,
          nombre: productoData.nombre || productoData.descripcion || "Sin nombre",
          costo: costo,
          stock: stock,
          costoTotal: costoTotal,
          categoria: productoData.categoria || productoData.categoriaId || "Sin categoría"
        });
      } else {
        console.log("⚠️ Producto con datos inválidos:", { id: doc.id, costo, stock });
      }
    });
