...
export async function GET(request) {
  try {
    ...
    // TEMPORAL: Si no hay ventas directas, usar ingresos relacionados con ventas
    if (ventasEfectivo === 0 && ventasVirtual === 0) {
      console.log("⚠️ No hay ventas directas, usando ingresos relacionados con ventas");
      ...
      ingresosSnapshot.forEach((doc) => {
        ...
        if (ingresoData.monto && !isNaN(parseFloat(ingresoData.monto))) {
          ...
          if (motivo.includes("Cobranza mesa") || motivo.includes("Takeaway") || motivo.includes("Delivery")) {
            ...
            console.log("💰 Ingreso convertido a venta:", {
              motivo,
              monto: total,
              tipo: "efectivo"
            });
