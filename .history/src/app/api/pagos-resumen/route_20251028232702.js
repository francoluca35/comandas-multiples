...
export async function GET(request) {
  try {
    ...
    // Calcular dinero virtual inicial (solo para referencia, se recalculará después)
    let virtualInicial = 0;
    dineroSnapshot.forEach((doc) => {
      const dineroData = doc.data();
      if (dineroData.Virtual && !isNaN(parseFloat(dineroData.Virtual))) {
        virtualInicial = parseFloat(dineroData.Virtual);
      }
    });
    ...
    // Calcular dinero actual real: apertura + ventas - egresos
    efectivoTotal = efectivoTotal + ventasEfectivo - egresosEfectivo;
    virtualTotal = virtualInicial + ventasVirtual - egresosVirtual;
