...
export async function GET(request) {
  try {
    ...
    console.log("🔍 API Proveedores - Obteniendo pagos a proveedores para restaurante:", restauranteId);
    ...
    try {
      ...
      console.log("ℹ️ Colección CajaRegistradora no existe aún para proveedores");
    }
    ...
    try {
      ...
      dineroSnapshot.forEach((doc) => {
        const data = doc.data();
        if (data.ProveedorVirtual && typeof data.ProveedorVirtual === 'object') {
          proveedores.push({
            id: doc.id,
      ...
      console.log("ℹ️ Colección Dinero no existe aún para proveedores virtuales");
    }
    ...
    console.log("✅ Pagos a proveedores obtenidos exitosamente:", {
      ...
      totalProveedores: proveedores.reduce((sum, proveedor) => {
        if (proveedor.historialPagos && typeof proveedor.historialPagos === 'object') {
          const proveedorTotal = Object.values(proveedor.historialPagos).reduce((proveedorSum, pago) => {
