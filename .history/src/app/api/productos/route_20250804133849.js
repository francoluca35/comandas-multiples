...
// GET - Obtener todos los productos o productos por subcategoría
  ...
  try {
    ...
    if (!mainCategoryId || !subCategoryId || !name || !price) {
      return NextResponse.json(
        {
          success: false,
          error: "Categoría principal, subcategoría, nombre y precio son obligatorios",
        },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (!productId || !mainCategoryId || !subCategoryId || !name || !price) {
      return NextResponse.json(
        {
          success: false,
          error: "ID del producto, categoría principal, subcategoría, nombre y precio son obligatorios",
        },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (!productId || !mainCategoryId || !subCategoryId) {
      return NextResponse.json(
        {
          success: false,
          error: "ID del producto, categoría principal y subcategoría son obligatorios",
        },
        { status: 400 }
      );
    }
