...
export async function POST(request) {
  try {
    ...
    if (!orderData) {
      ...
      return NextResponse.json({ error: "Falta orderData" }, { status: 400 });
    }

    console.log(
      "API de tickets: Obteniendo datos del restaurante:",
      restaurantId
    );
    ...
    console.log(
      "API de tickets: Datos del restaurante obtenidos:",
      restaurantData
    );
    ...
    if (!orderData.productos || !Array.isArray(orderData.productos)) {
      console.error(
        "API de tickets: Falta información de productos en orderData"
      );
      return NextResponse.json(
    ...
    if (!ticketContent) {
      console.error(
        "API de tickets: No se pudo generar el contenido del ticket"
      );
      return NextResponse.json(
...
function generateTicketContent(restaurantData, orderData) {
  try {
    ...
    if (
      ...
      if (orderData.mesa === "DELIVERY" && orderData.direccion) {
        content += `<div style="font-size: 10px; color: #000000; margin-top: 2px;">Dirección: ${orderData.direccion}</div>`;
      }
      content += `</div>`;
