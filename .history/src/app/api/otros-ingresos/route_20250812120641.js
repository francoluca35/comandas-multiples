...
export async function GET(request) {
  try {
    ...
    console.log(
      "🔍 API Otros Ingresos - Obteniendo otros ingresos para restaurante:",
      restauranteId
    );

    // Obtener todos los documentos de la colección OtrosIngresos
    const otrosRef = collection(
      db,
      "restaurantes",
      restauranteId,
      "OtrosIngresos"
    ...
    try {
      ...
      console.log(
        "ℹ️ Colección OtrosIngresos no existe aún, retornando array vacío"
      );
      return NextResponse.json({
        success: true,
        data: [],
    ...
    console.log("✅ Otros ingresos obtenidos exitosamente:", {
      ...
      totalOtros: otros.reduce((sum, otro) => {
        if (otro.historialOtros && typeof otro.historialOtros === "object") {
          const otroTotal = Object.values(otro.historialOtros).reduce(
            (otroSum, pago) => {
