...
export async function POST(request) {
  try {
    ...
    const secretKey =
      ...
      "7e9343f241f1e615cb843e3c5ee77c51557b31de987b92d37509c459b861b073";

    // TODO: Implementar verificación de firma completa
    // Por ahora, verificamos que venga de MercadoPago por otros medios
    if (!body || !body.type) {
...
async function handleApprovedPayment(
  ...
  try {
    ...
    try {
      ...
      console.warn("⚠️ No se pudo registrar el ingreso automático:", ingresoError.message);
    }
...
async function registrarIngresoAutomaticoWebhook(
  ...
  try {
    ...
    const ingresosRef = collection(db, "restaurantes", restaurantId, "Ingresos");
    ...
    console.log("✅ Ingreso registrado automáticamente desde webhook con ID:", docRef.id);

    // Actualizar dinero virtual
    await actualizarDineroVirtualIngreso(restaurantId, monto, nuevoIngreso.motivo, new Date());

    return docRef.id;
  } catch (error) {
    console.error("❌ Error registrando ingreso automático desde webhook:", error);
    throw error;
  }
}
