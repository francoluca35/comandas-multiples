...
const liberarMesa = async (restauranteId, mesaId) => {
  try {
    ...
    mesasSnapshot.forEach((doc) => {
      ...
      console.log(`🔍 Verificando mesa: ID=${doc.id}, numero=${mesaData.numero}`);
      
      // Comparar por número de mesa
      if (mesaData.numero === mesaId || mesaData.numero === mesaId.toString()) {
        mesaEncontrada = doc;
        mesaIdEncontrado = doc.id;
        console.log(`✅ Mesa encontrada: ${doc.id} con número ${mesaData.numero}`);
      }
    });
    ...
    console.log(`✅ Mesa ${mesaIdEncontrado} (número ${mesaId}) liberada exitosamente`);
    return true;
  } catch (error) {
