...
export async function POST(request) {
  try {
    ...
    if (!restaurantData?.mercadopago?.accessToken) {
      return NextResponse.json(
        { error: "Restaurante no tiene Mercado Pago configurado. Por favor, configura las credenciales en la sección de configuración." },
        { status: 400 }
      );
    }
    ...
    if (!restaurantData.mercadopago.isActive) {
      return NextResponse.json(
        { error: "Configuración de Mercado Pago inactiva. Por favor, verifica la configuración." },
        { status: 400 }
      );
    }
