...
export async function POST(request) {
  try {
    ...
    const { nombre, email, telefono, direccion, codigoActivacion, cantidadUsuarios, conFinanzas } = body;

    // Validar datos requeridos
    if (!nombre || !email || !telefono || !direccion || !codigoActivacion || !cantidadUsuarios) {
    ...
    await setDoc(doc(db, "restaurantes", restauranteId, "menus", "comida", "subcategorias", "platos_principales"), {
      nombre: "Platos Principales",
      activo: true,
      fechaCreacion: new Date().toISOString(),
    });

    await setDoc(doc(db, "restaurantes", restauranteId, "menus", "comida", "subcategorias", "entradas"), {
      nombre: "Entradas",
      activo: true,
      fechaCreacion: new Date().toISOString(),
    });

    await setDoc(doc(db, "restaurantes", restauranteId, "menus", "comida", "subcategorias", "postres"), {
      nombre: "Postres",
      activo: true,
      fechaCreacion: new Date().toISOString(),
    });
    ...
    await setDoc(doc(db, "restaurantes", restauranteId, "menus", "bebidas", "subcategorias", "con_alcohol"), {
      nombre: "Con Alcohol",
      activo: true,
      fechaCreacion: new Date().toISOString(),
    });

    await setDoc(doc(db, "restaurantes", restauranteId, "menus", "bebidas", "subcategorias", "sin_alcohol"), {
      nombre: "Sin Alcohol",
      activo: true,
      fechaCreacion: new Date().toISOString(),
    });
