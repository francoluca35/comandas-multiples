...
const crearColeccionesIniciales = async (restauranteId) => {
  ...
  console.log("📁 Creando colecciones iniciales para restaurante:", restauranteId);

  // 1. CajaRegistradora - Documento inicial de caja
  await setDoc(doc(db, "restaurantes", restauranteId, "CajaRegistradora", "caja_principal"), {
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "Dinero", "dinero_actual"), {
    monto: 0,
    moneda: "ARS",
    fechaCreacion: timestamp,
    fechaActualizacion: timestamp,
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "Mpagos", "configuracion"), {
    activo: false,
    accessToken: "",
    publicKey: "",
    fechaCreacion: timestamp,
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "ServiciosComercio", "servicios_basicos"), {
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "SueldoEmpleados", "empleado_admin"), {
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "bebidas", "bebida_ejemplo"), {
    nombre: "Agua",
    precio: 500,
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "mensajes", "bienvenida"), {
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "pedidosCocina", "configuracion"), {
    activo: true,
    notificaciones: true,
    fechaCreacion: timestamp,
  });
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "stock", "producto_ejemplo"), {
  ...
  await setDoc(doc(db, "restaurantes", restauranteId, "usuarios", "usuario_admin"), {
    nombre: "Administrador",
    email: "",
    rol: "admin",
    activo: true,
...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Restaurante creado exitosamente con todas las colecciones iniciales",
      restaurante: {
        id: restauranteId,
        ...restauranteData,
      },
