...
export async function POST(request) {
  try {
    ...
    const categoryExists = categoriesSnapshot.docs.some(doc => 
      doc.id.toLowerCase() === categoryName.toLowerCase()
    );
    ...
    const categoryRef = doc(db, 'restaurantes', restaurantId, 'menus', categoryName);
...
export async function PUT(request) {
  try {
    ...
    const oldCategoryRef = doc(db, 'restaurantes', restaurantId, 'menus', oldName);
    ...
    const newCategoryRef = doc(db, 'restaurantes', restaurantId, 'menus', newCategoryName);
    ...
    const productsRef = collection(db, 'restaurantes', restaurantId, 'menus', oldName, 'items');
    ...
    for (const productDoc of productsSnapshot.docs) {
      ...
      const newProductRef = collection(db, 'restaurantes', restaurantId, 'menus', newCategoryName, 'items');
      ...
      const oldProductRef = doc(db, 'restaurantes', restaurantId, 'menus', oldName, 'items', productDoc.id);
...
export async function DELETE(request) {
  try {
    ...
    const productsRef = collection(db, 'restaurantes', restaurantId, 'menus', categoryName, 'items');
    const productsSnapshot = await getDocs(productsRef);
    
    // Eliminar todos los productos de la categoría
    for (const productDoc of productsSnapshot.docs) {
      const productRef = doc(db, 'restaurantes', restaurantId, 'menus', categoryName, 'items', productDoc.id);
