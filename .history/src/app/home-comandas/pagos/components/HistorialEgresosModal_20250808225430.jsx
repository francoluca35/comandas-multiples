...
function HistorialEgresosModal({ isOpen, onClose }) {
  ...
  const fetchHistorial = async () => {
    try {
      ...
      const [aliviosRes, comprasRes, sueldosRes, retirosRes, proveedoresRes] = await Promise.all([
      ...
      const [aliviosData, comprasData, sueldosData, retirosData, proveedoresData] = await Promise.all([
        aliviosRes.json(),
        comprasRes.json(),
        sueldosRes.json(),
        retirosRes.json(),
      ...
      <div className="bg-slate-800 rounded-2xl border border-slate-700/50 p-6 w-[1200px] max-h-[90vh] overflow-y-auto shadow-2xl shadow-slate-900/50">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-white">Historial de Egresos</h2>
          </div>
          <button
            onClick={handleClose}
                ...
                {loading ? (
                  ...
                  datosFiltrados.map((item, index) => (
                    <tr key={index} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-300">
                        {item.motivo || item.producto || item.nombreEmpleado || item.proveedor || "N/A"}
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-300">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400">
                  {formatCurrency(datosFiltrados.reduce((sum, item) => sum + (parseFloat(item.total || item.importe || item.costo || 0)), 0))}
                </div>
                <div className="text-sm text-slate-400">Total en efectivo</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-400">
                  {formatCurrency(datosFiltrados.filter(item => item.categoria === "Virtual").reduce((sum, item) => sum + (parseFloat(item.total || item.importe || item.costo || 0)), 0))}
                </div>
