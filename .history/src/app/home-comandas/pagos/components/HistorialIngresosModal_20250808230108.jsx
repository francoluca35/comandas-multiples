...
function HistorialIngresosModal({ isOpen, onClose }) {
  ...
  const fetchHistorial = async () => {
    try {
      ...
      const [ingresosRes, ventasRes, otrosRes, depositosRes] = await Promise.all([
      ...
      const [ingresosData, ventasData, otrosData, depositosData] = await Promise.all([
        ingresosRes.json(),
        ventasRes.json(),
        otrosRes.json(),
        depositosRes.json()
                    ...
                    <tr key={index} className="hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-300">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          item.tipo === "Ingreso" ? "bg-green-500/20 text-green-400" :
                          item.tipo === "Venta" ? "bg-blue-500/20 text-blue-400" :
                          item.tipo === "Otro" ? "bg-yellow-500/20 text-yellow-400" :
                      ...
                      <td className="px-4 py-3 text-sm text-slate-300 text-right font-semibold">
                        {formatCurrency(item.monto || item.total || 0)}
                      </td>
                    </tr>
                  ))
                )}
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-green-400">
                  {formatCurrency(datosFiltrados.filter(item => item.categoria === "Efectivo").reduce((sum, item) => sum + (parseFloat(item.monto || item.total || 0)), 0))}
                </div>
                <div className="text-sm text-slate-400">Total en efectivo</div>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold text-purple-400">
                  {formatCurrency(datosFiltrados.filter(item => item.categoria === "Virtual").reduce((sum, item) => sum + (parseFloat(item.monto || item.total || 0)), 0))}
                </div>
