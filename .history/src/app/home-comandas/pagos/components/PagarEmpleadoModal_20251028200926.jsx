  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!dineroSnapshot.empty) {
        ...
        const efectivoActual = parseFloat(dineroData.Efectivo || 0);
        ...
        if (formData.formaPago === "efectivo") {
          updateData.Efectivo = (efectivoActual - monto).toString();
        } else {
          updateData.Virtual = (virtualActual - monto).toString();
        }

        await updateDoc(dineroDoc.ref, updateData);
      } else {
        // Crear documento si no existe
        const newData = {
          Efectivo: formData.formaPago === "efectivo" ? (efectivoDisponible - monto).toString() : "0",
          Virtual: formData.formaPago === "virtual" ? (virtualDisponible - monto).toString() : "0",
          fechaCreacion: serverTimestamp(),
          fechaActualizacion: serverTimestamp()
        };
        await addDoc(dineroRef, newData);
      }
