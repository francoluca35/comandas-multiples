...
function Rendimiento({ onToggle, isExpanded, data, formatDinero, getEfectivoTotal, getVirtualTotal, getTotalIngresos, getTotalEgresos, getVentasEfectivo, getVentasVirtual }) {
  ...
  const ingresosTotales = getTotalIngresos();
  ...
  // Calcular dinero actual (efectivo + virtual)
  const dineroActualEfectivo = data?.dineroActual?.efectivo || 0;
  const dineroActualVirtual = data?.dineroActual?.virtual || 0;
  const dineroActualTotal = dineroActualEfectivo + dineroActualVirtual;
  
  // Calcular ingresos totales reales (dinero actual + ingresos registrados + ventas)
  const ingresosReales = dineroActualTotal + ingresosTotales + ventasTotales;
    ...
    <div className="bg-slate-800/50 rounded-2xl border border-slate-700/50 p-4 shadow-2xl shadow-slate-900/50">
      ...
      {isExpanded && (
        <>
          ...
          <div className="text-xs text-slate-400 mb-4">Dinero Actual + Ingresos + Ventas</div>
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-300">• Dinero Actual:</span>
              <span className="text-green-400 font-medium">
                {formatDinero(dineroActualTotal)}
              </span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-slate-300">• Ingresos Registrados:</span>
              <span className="text-green-400 font-medium">
