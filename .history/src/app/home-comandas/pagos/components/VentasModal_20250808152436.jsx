...
function VentasModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-2xl border border-slate-700 p-6 w-full max-w-4xl max-h-[90vh] overflow-hidden">
        {/* Header */}
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center space-x-3">
                      <div className={`w-3 h-3 rounded-full ${
                        venta.tipo === "Restaurante" ? "bg-green-500" :
                        venta.tipo === "Takeaway" ? "bg-blue-500" : "bg-purple-500"
                      }`}></div>
                      <span className="text-white font-medium">{venta.tipo}</span>
                      {venta.mesa && <span className="text-slate-400">- {venta.mesa}</span>}
                      {venta.cliente && <span className="text-slate-400">- {venta.cliente}</span>}
                    </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center space-x-4">
                      ...
                      {venta.direccion && (
                        <span className="text-slate-400">• {venta.direccion}</span>
                      )}
                    </div>
                  </div>
