...
function Egresos({ onToggle, isExpanded }) {
  ...
  const { getTotalSueldos, getCantidadSueldos, sueldos, fetchSueldos } =
    useSueldos();
  ...
  const [showHistorialEgresosModal, setShowHistorialEgresosModal] =
    useState(false);

  // Memoizar valores individuales para evitar recálculos
  const totalAlivios = useMemo(() => getTotalAlivios || 0, [getTotalAlivios]);
  const totalCompras = useMemo(() => getInversionTotal || 0, [getInversionTotal]);
  const totalSueldos = useMemo(() => getTotalSueldos || 0, [getTotalSueldos]);
  const cantidadSueldos = useMemo(() => getCantidadSueldos || 0, [getCantidadSueldos]);

  // Calcular gastos totales con memoización
  const gastosTotales = useMemo(() => {
    try {
      const total = totalAlivios + totalCompras + totalSueldos;
  ...
  }, [totalAlivios, totalCompras, totalSueldos]);
  ...
  return (
    <div className="bg-slate-800/50 rounded-2xl border border-slate-700/50 p-4 shadow-2xl shadow-slate-900/50">
      ...
      {isExpanded && (
        <>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-white text-sm font-medium">
                  Gasto en sueldos ({cantidadSueldos} pagados)
                </span>
              </div>
              <span className="text-lg font-bold text-yellow-400">
                {formatDinero(totalSueldos)}
              </span>
            </div>
          ...
          <div className="flex justify-center mb-4">
            <button
              onClick={() => setShowHistorialEgresosModal(true)}
              className="w-full bg-slate-600 text-white font-medium py-2 px-3 rounded-lg hover:bg-slate-700 transition-all duration-300 flex items-center justify-center space-x-2"
            >
              <svg
                className="w-4 h-4"
                fill="none"
      ...
      {/* Modal de Pagar Empleado */}
      <PagarEmpleadoModal
        isOpen={showPagarEmpleadoModal}
        onClose={() => setShowPagarEmpleadoModal(false)}
        onSuccess={() => {
          console.log("✅ Pago a empleado creado exitosamente");
          // Recargar datos después de crear un pago a empleado
          fetchSueldos();
        }}
      />

      {/* Modal de Historial de Egresos */}
      <HistorialEgresosModal
        isOpen={showHistorialEgresosModal}
        onClose={() => setShowHistorialEgresosModal(false)}
      />
    </div>
  );
}

export default memo(Egresos);
