...
function Rendimiento({ onToggle, isExpanded, data, formatDinero, getEfectivoTotal, getVirtualTotal, getTotalIngresos, getTotalEgresos, getVentasEfectivo, getVentasVirtual }) {
  ...
  // Los ingresos totales en rendimiento deben mostrar el dinero actual (ventas - egresos ya descontados)
  const ingresosReales = getEfectivoTotal() + getVirtualTotal();
  
  // Ganancia de venta: solo el dinero acumulado por ventas (sin descontar egresos)
  const gananciaVenta = ventasTotales;
  
  // Calcular ganancia o pérdida: ingresos totales - egresos totales
  const gananciaPerdida = ingresosReales - egresosTotales;
    ...
    <div className="bg-slate-800/50 rounded-2xl border border-slate-700/50 p-4 shadow-2xl shadow-slate-900/50">
      ...
      {isExpanded && (
        <>
          ...
          <div className="text-xs text-slate-400 mb-4">Dinero Disponible</div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-lg font-bold text-blue-400">
              {formatDinero(gananciaVenta)}
            </span>
          </div>
          <div className="text-xs text-slate-400 mb-4">Dinero acumulado por ventas</div>
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center justify-between text-xs border-t border-slate-600 pt-1 mt-1">
              <span className="text-slate-300 font-semibold">• Total Rendimiento:</span>
              <span className="text-green-400 font-bold">
                {formatDinero(dineroActualTotal)}
              </span>
            </div>
            <div className="flex items-center justify-between text-xs border-t border-slate-600 pt-1 mt-1">
              <span className="text-slate-300 font-semibold">• Ganancia o Pérdida:</span>
              <span className={`font-bold ${rendimientoColor}`}>
                {formatDinero(rendimiento)}
              </span>
            </div>
          </div>
        </>
