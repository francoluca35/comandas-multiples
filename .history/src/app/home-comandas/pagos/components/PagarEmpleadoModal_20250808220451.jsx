...
function PagarEmpleadoModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    const nuevoTotal = formData.presentismo 
      ...
      : nuevoSubtotal;
    setTotal(nuevoTotal);
  }, [formData.horasTrabajadas, formData.diasTrabajados, formData.sueldoPorHora, formData.presentismo]);
  ...
  const handleDateSelection = (date) => {
    ...
    setSelectedDates((prev) => {
      if (prev.includes(dateStr)) {
        ...
        setFormData(prevForm => ({ ...prevForm, diasTrabajados: newDates.length.toString() }));
        return newDates;
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.diasTrabajados || parseFloat(formData.diasTrabajados) <= 0) {
  ...
  const generateCalendarDates = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (date <= today) { // Solo mostrar fechas hasta hoy
        dates.push(date);
      }
    }
    return dates;
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleInputChange("nombreEmpleado", e.target.value)}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleInputChange("sueldoPorHora", e.target.value)}
              placeholder="1000.00"
              className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg text-slate-100 placeholder-slate-400 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500/50 transition-all duration-200"
              required
            />
          ...
          <div className="flex items-center space-x-3">
            <input
              ...
              onChange={(e) => handleInputChange("presentismo", e.target.checked)}
              className="w-4 h-4 text-yellow-500 bg-slate-700 border-slate-600 rounded focus:ring-yellow-500 focus:ring-2"
            />
            <label htmlFor="presentismo" className="text-sm font-semibold text-slate-200">
          ...
          <div className="bg-slate-700/30 rounded-lg p-4 space-y-2">
            ...
            {formData.presentismo && (
              <div className="flex justify-between items-center">
                <span className="text-slate-300 text-sm">Presentismo (20%):</span>
