...
function Egresos({ onToggle, isExpanded }) {
  ...
  const { getInversionTotal, inversionTotal, fetchInversionTotal } = useInversionTotal();
  ...
  // Cargar datos cuando el componente se expande
  useEffect(() => {
    if (isExpanded) {
      fetchAlivios();
      fetchInversionTotal();
    }
  }, [isExpanded, fetchAlivios, fetchInversionTotal]);
  ...
  const calcularGastosTotales = () => {
    ...
    console.log("💰 Gastos totales calculados:", { totalAlivios, totalCompras, total });
    return total;
  };
          ...
          <div className="space-y-2 mb-4">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-600/30">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-white text-sm font-medium">Gastos Totales</span>
              </div>
