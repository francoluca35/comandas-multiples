...
function Ingresos({ onToggle, isExpanded }) {
  ...
  const [showHistorialIngresosModal, setShowHistorialIngresosModal] =
    useState(false);
    ...
    <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl border border-slate-700/50 p-4 shadow-2xl shadow-slate-900/50">
      ...
      <div className="mt-4">
        ...
        <div className="grid grid-cols-1 gap-2 mb-4">
          <button
            onClick={() => setShowDepositarModal(true)}
            className="bg-blue-500 text-white font-medium py-2 px-2 rounded-lg hover:bg-blue-600 transition-all duration-300 flex items-center justify-center space-x-1"
          >
            <svg
              className="w-3 h-3"
      ...
      <OtrosModal
        isOpen={showOtrosModal}
        onClose={() => setShowOtrosModal(false)}
        onSuccess={() => {
          console.log("✅ Ingreso extra creado exitosamente");
          fetchIngresos(); // Recargar datos después de crear un ingreso
        }}
      />

      <DepositarModal
        isOpen={showDepositarModal}
        onClose={() => setShowDepositarModal(false)}
        onSuccess={() => {
          console.log("✅ Depósito registrado exitosamente");
          fetchIngresos(); // Recargar datos después de registrar un depósito
        }}
      />
