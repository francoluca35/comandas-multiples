...
function RetirarEfectivoModal({ isOpen, onClose, onSuccess, data, formatDinero }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Actualizar el dinero virtual (restar del efectivo disponible)
      ...
      if (!dineroSnapshot.empty) {
        ...
        await updateDoc(dineroDoc.ref, {
          Efectivo: (efectivoActual - monto).toString(),
          ultimaActualizacion: serverTimestamp()
        });
      } else {
        // Crear registro de dinero si no existe
        await addDoc(dineroRef, {
          Efectivo: (efectivoDisponible - monto).toString(),
          Virtual: "0",
          ultimaActualizacion: serverTimestamp()
        });
      }
