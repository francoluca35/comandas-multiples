...
function ProductosContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentView === "menu") {
      ...
      if (
        selectedMainCategory &&
        selectedMainCategory.name.toLowerCase().includes("bebida")
      ) {
        showAlert(
          "❌ Error de Validación",
    ...
    try {
      ...
      if (editingProduct) {
        ...
        showAlert(
          ...
          `El ${
            currentView === "menu" ? "producto del menú" : "item del inventario"
          } "${formData.name}" ha sido actualizado correctamente.`,
          "success"
        );
        resetForm();
        setShowForm(false);
      ...
      showAlert(
        "❌ Error al Guardar",
        `Hubo un error al ${editingProduct ? "actualizar" : "crear"} el ${
          currentView === "menu" ? "producto" : "item del inventario"
  ...
  const handleMainCategorySubmit = async (e) => {
    ...
    try {
      ...
      showAlert(
        "❌ Error al Guardar",
        `Hubo un error al ${
          editingMainCategory ? "actualizar" : "crear"
        } la categoría: ${err.message}`,
        "error"
      );
    }
  };
                ...
                <div className="bg-slate-600/20 border border-slate-500/30 rounded-lg p-3 mb-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <p className="text-slate-200 text-sm">
                      <strong>Nota:</strong> Las bebidas se crean desde la vista
                      "Inventario". Aquí solo puedes crear platos y postres.
                    </p>
                  </div>
                </div>
              )}
