...
function ProductosContent() {
  ...
  const getFilteredProducts = () => {
    if (currentView === "menu") {
      // Productos del menú (platos, postres) + bebidas del inventario - NO materia prima, NO bebidas del menú
      // Esto evita duplicación: solo bebidas del inventario, no bebidas creadas directamente como productos
      return products.filter(product => 
        (product.origen !== "inventario" && product.tipo !== "bebida") || 
        (product.origen === "inventario" && product.tipo === "bebida")
      );
    } else {
      // Solo inventario (materia prima) - NO bebidas
      return products.filter(product => 
        product.origen === "inventario" && product.tipo !== "bebida"
      );
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Validar que no se puedan crear bebidas desde la vista de productos
    if (currentView === "menu") {
      const selectedMainCategory = mainCategories.find(cat => cat.id === formData.mainCategoryId);
      if (selectedMainCategory && selectedMainCategory.name.toLowerCase().includes('bebida')) {
        showAlert(
          "❌ Error de Validación",
          "Las bebidas no se pueden crear desde la vista de productos. Deben crearse desde la vista de inventario.",
          "error"
        );
        return;
      }
  ...
  const handleMainCategorySubmit = async (e) => {
    ...
    try {
      if (editingMainCategory) {
        await updateMainCategory(editingMainCategory.id, mainCategoryFormData.name);
        showAlert(
          "✅ Categoría Actualizada",
          `La categoría principal "${mainCategoryFormData.name}" ha sido actualizada correctamente.`,
          "success"
        );
      } else {
        ...
        showAlert(
          "✅ Categoría Creada",
          `La categoría principal "${mainCategoryFormData.name}" ha sido creada correctamente.`,
          "success"
        );
      }
      ...
      showAlert(
        "❌ Error al Guardar",
        `Hubo un error al ${editingMainCategory ? "actualizar" : "crear"} la categoría: ${err.message}`,
        "error"
      );
    }
  };
    ...
    try {
      if (editingSubCategory) {
        ...
        showAlert(
          ...
          `La subcategoría "${subCategoryFormData.name}" ha sido actualizada correctamente.`,
          "success"
        );
      } else {
        ...
        showAlert(
          "✅ Subcategoría Creada",
          `La subcategoría "${subCategoryFormData.name}" ha sido creada correctamente.`,
          "success"
        );
      }
      ...
      showAlert(
        "❌ Error al Guardar",
        `Hubo un error al ${editingSubCategory ? "actualizar" : "crear"} la subcategoría: ${err.message}`,
        "error"
      );
    }
  };
  ...
  // Funciones para manejar categorías principales
  const handleEditMainCategory = (category) => {
    setEditingMainCategory(category);
    setMainCategoryFormData({ name: category.name });
    ...
    showAlert(
      ...
      async () => {
        try {
          ...
          showAlert(
            "❌ Error al Eliminar",
            `Hubo un error al eliminar la categoría: ${err.message}`,
            "error"
          );
        }
      },
  ...
  const handleEditSubCategory = (subCategory, mainCategoryId) => {
    setEditingSubCategory({ ...subCategory, mainCategoryId });
  ...
  const handleDeleteSubCategory = async (subCategory, mainCategoryId) => {
    showAlert(
      "⚠️ Confirmar Eliminación",
      ...
      async () => {
        try {
          await deleteSubCategory(mainCategoryId, subCategory.id);
          showAlert(
            "✅ Subcategoría Eliminada",
            `La subcategoría "${subCategory.name}" ha sido eliminada correctamente.`,
            "success"
          );
        } catch (err) {
          ...
          showAlert(
            "❌ Error al Eliminar",
            `Hubo un error al eliminar la subcategoría: ${err.message}`,
            "error"
          );
        }
      },
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (currentView === "menu") {
      // Productos del menú (platos, postres) + bebidas del inventario - NO materia prima
      passesViewFilter = (product.origen !== "inventario" && product.tipo !== "bebida") || 
                        (product.origen === "inventario" && product.tipo === "bebida");
    } else {
      // Solo inventario (materia prima) - NO bebidas
      passesViewFilter = product.origen === "inventario" && product.tipo !== "bebida";
  ...
  return (
    <div className="flex h-screen bg-slate-900">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                ...
                className="bg-slate-600 text-white px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center sm:justify-start space-x-2 hover:bg-slate-700 text-sm sm:text-base"
              >
                {getIcon("add")}
          ...
          <div className="mb-4">
            <p className="text-slate-400 text-sm">
              {currentView === "menu" 
                ...
                : "Items del inventario para control de stock (materia prima)"
              }
            </p>
          </div>
          {/* Error Display */}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
                <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                  ...
                  <select
                    ...
                    className="bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 focus:outline-none focus:border-slate-400 text-sm sm:text-base"
                  >
                ...
                {selectedMainCategory && (
                  <div className="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-4">
                    ...
                    <select
                      ...
                      className="bg-slate-800 text-white px-3 py-2 rounded-lg border border-slate-600 focus:outline-none focus:border-slate-400 text-sm sm:text-base"
                    >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-lg p-4 sm:p-6 w-full max-w-md sm:max-w-lg mx-auto max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
              {currentView === "menu" && (
                <div className="bg-slate-600/20 border border-slate-500/30 rounded-lg p-3 mb-4">
                  <div className="flex items-center space-x-2">
                    <svg className="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p className="text-slate-200 text-sm">
                      <strong>Nota:</strong> Las bebidas se crean desde la vista "Inventario". Aquí solo puedes crear platos y postres.
                    </p>
                  </div>
                </div>
              )}
              <div>
                ...
                <select
                  ...
                  required
                  className="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 focus:outline-none focus:border-slate-400 text-sm sm:text-base"
                >
                  <option value="">Seleccionar categoría principal</option>
                  {mainCategories
                    .filter(category => {
                      // En vista de productos, excluir categorías de bebidas
                      if (currentView === "menu") {
                        return !category.name.toLowerCase().includes('bebida');
                      }
                      // En vista de inventario, mostrar todas las categorías
                      return true;
                    })
                    .map((category) => (
                      <option key={category.id} value={category.id}>
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-slate-600 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-slate-700 disabled:opacity-50 text-sm sm:text-base"
                >
                  {loading
                    ? "Guardando..."
                    : editingProduct
          ...
          <div className="bg-slate-800 rounded-lg p-6 w-full max-w-md mx-4">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-white">
                {editingMainCategory ? "Editar Categoría Principal" : "Nueva Categoría Principal"}
              </h2>
              <button
                onClick={() => {
                  setShowMainCategoryForm(false);
            ...
            <form onSubmit={handleMainCategorySubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-700 text-white px-3 py-2 rounded-lg border border-slate-600 focus:outline-none focus:border-slate-400"
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 bg-slate-600 text-white px-4 py-2 rounded-lg hover:bg-slate-700 disabled:opacity-50"
                >
      ...
      {showCategoryManager && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-lg p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-white">Categorías Principales</h3>
                  <button
                    onClick={() => {
                      setEditingMainCategory(null);
                      setMainCategoryFormData({ name: "" });
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {mainCategories.map((category) => (
                    <div key={category.id} className="bg-slate-700/50 rounded-lg p-4 flex items-center justify-between">
                      <span className="text-white font-medium">{category.name}</span>
                      <div className="flex space-x-2">
                        <button
                          onClick={() => handleEditMainCategory(category)}
                          className="text-slate-400 hover:text-slate-300 p-2 rounded-lg hover:bg-slate-600/50"
                        ...
                        <button
                          ...
                          className="text-red-400 hover:text-red-300 p-2 rounded-lg hover:bg-slate-600/50"
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-white">Subcategorías</h3>
                  <button
                    onClick={() => {
                      setEditingSubCategory(null);
                      setSubCategoryFormData({ name: "", mainCategoryId: "" });
                ...
                <div className="space-y-4">
                  {Object.entries(subCategories).map(([mainCategoryId, subCats]) => {
                    const mainCategory = mainCategories.find(cat => cat.id === mainCategoryId);
                    return (
                      <div key={mainCategoryId} className="bg-slate-700/30 rounded-lg p-4">
                        <div className="text-slate-300 font-medium mb-3 flex items-center">
                          <span className="w-2 h-2 bg-slate-400 rounded-full mr-2"></span>
                          {mainCategory?.name || mainCategoryId}
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                          {subCats.map((subCategory) => (
                            <div key={subCategory.id} className="bg-slate-600/50 rounded-lg p-3 flex items-center justify-between">
                              <span className="text-white text-sm">{subCategory.name}</span>
                              <div className="flex space-x-1">
                                <button
                                  onClick={() => handleEditSubCategory(subCategory, mainCategoryId)}
                                  className="text-slate-400 hover:text-slate-300 p-1 rounded hover:bg-slate-500/50"
                                  title="Editar subcategoría"
                                >
                                  {getIcon("edit")}
                                </button>
                                <button
                                  onClick={() => handleDeleteSubCategory(subCategory, mainCategoryId)}
                                  className="text-red-400 hover:text-red-300 p-1 rounded hover:bg-slate-500/50"
