        ...
        <div className="flex-1 p-4 sm:p-6">
          ...
          <div className="bg-slate-800 p-4 rounded-lg mb-6">
            <div className="flex flex-col space-y-4">
              ...
              {/* Lista de categorías con botones de editar/eliminar */}
              <div className="border-t border-slate-700 pt-4">
                <h3 className="text-white font-medium text-sm mb-3">Gestionar Categorías</h3>
                
                {/* Categorías principales */}
                <div className="space-y-2">
                  <h4 className="text-slate-300 text-xs font-medium uppercase tracking-wide">Categorías Principales</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                    {mainCategories.map((category) => (
                      <div key={category.id} className="bg-slate-700/50 rounded-lg p-3 flex items-center justify-between">
                ...
                {Object.keys(subCategories).length > 0 && (
                  <div className="space-y-2 mt-4">
                    <h4 className="text-slate-300 text-xs font-medium uppercase tracking-wide">Subcategorías</h4>
                    <div className="space-y-2">
                      {Object.entries(subCategories).map(([mainCategoryId, subCats]) => {
                        const mainCategory = mainCategories.find(cat => cat.id === mainCategoryId);
                        return (
                          <div key={mainCategoryId} className="bg-slate-700/30 rounded-lg p-3">
                            <div className="text-slate-400 text-xs font-medium mb-2">
                              {mainCategory?.name || mainCategoryId}
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {subCats.map((subCategory) => (
                                <div key={subCategory.id} className="bg-slate-600/50 rounded p-2 flex items-center justify-between">
