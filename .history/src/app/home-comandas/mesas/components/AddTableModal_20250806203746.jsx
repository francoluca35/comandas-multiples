...
function AddTableModal({
  ...
  const handleSubmit = async (e) => {
    ...
    const existingTable = tables.find(table => table.numero === formData.numero);
    if (existingTable) {
  ...
  const handleIntelligentOption = async (option) => {
    try {
      if (option === "create-multiple") {
        ...
        await createMultipleTables(startNumber, endNumber, intelligentOptions.targetLocation);
        
        alert(`Se crearon ${endNumber - startNumber + 1} mesas en ${intelligentOptions.targetLocation === "adentro" ? "Adentro" : "Afuera"}`);
      } else if (option === "reorder") {
        // Reordenar mesas existentes
        await reorderTablesForLocation(intelligentOptions.targetNumber, intelligentOptions.targetLocation);
        
        // Crear la mesa específica
        const newTable = {
          numero: String(intelligentOptions.targetNumber).padStart(2, "0"),
        ...
        await createMultipleTables(intelligentOptions.targetNumber, intelligentOptions.targetNumber, intelligentOptions.targetLocation);
        
        alert(`Se reordenaron las mesas y se creó la mesa ${intelligentOptions.targetNumber} en ${intelligentOptions.targetLocation === "adentro" ? "Adentro" : "Afuera"}`);
      }
        ...
        <div className="relative top-20 mx-auto p-8 border border-slate-700/50 w-96 shadow-2xl rounded-2xl bg-slate-800/90 backdrop-blur-sm">
          <div className="mt-3">
            ...
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-white">Opción Inteligente</h3>
              <button
                onClick={() => {
                  setShowIntelligentModal(false);
                  setIntelligentOptions(null);
            ...
            <div className="space-y-4">
              <p className="text-slate-300 text-sm">
                ...
                , pero actualmente solo hay hasta la mesa {intelligentOptions.lastNumberInLocation}.
              </p>

              <div className="bg-slate-700/50 rounded-lg p-4 space-y-3">
                <h4 className="font-semibold text-white">Opciones disponibles:</h4>
                ...
                >
                  <div className="font-semibold text-blue-400">Opción 1: Crear mesas faltantes</div>
                  <div className="text-sm text-slate-300 mt-1">
                    Crear {intelligentOptions.missingTables} mesas adicionales para completar la secuencia hasta la mesa {intelligentOptions.targetNumber}
                  </div>
                </button>
                ...
                >
                  <div className="font-semibold text-orange-400">Opción 2: Reordenar mesas existentes</div>
                  <div className="text-sm text-slate-300 mt-1">
                    Mover {intelligentOptions.missingTables} mesas de{" "}
                    {intelligentOptions.otherLocation === "adentro" ? "Adentro" : "Afuera"} para hacer espacio
                  </div>
                </button>
              </div>
            </div>
