  ...
  const handleIntelligentOption = async (option) => {
    try {
      if (option === "create-multiple") {
        ...
        // Buscar la mesa específica que necesitamos
        const targetTableNumber = String(intelligentOptions.targetNumber).padStart(2, "0");
        const otherLocation = intelligentOptions.targetLocation === "adentro" ? "afuera" : "adentro";
        
        // Buscar la mesa en la otra ubicación
        const tableToMove = tables.find(table => 
          table.numero === targetTableNumber && table.lugar === otherLocation
        );
        
        if (!tableToMove) {
          alert(`No se encontró la mesa ${intelligentOptions.targetNumber} en ${otherLocation === "adentro" ? "Adentro" : "Afuera"}`);
          return;
        }
        
        // Obtener el último número en la ubicación objetivo
        const lastNumberInTarget = getLastTableNumberInLocation(intelligentOptions.targetLocation);
        const newNumber = String(lastNumberInTarget + 1).padStart(2, "0");
        
        // Mover la mesa específica
        ...
        const remainingTablesInLocation = tables
          .filter(table => 
            ...
            table.id !== tableToMove.id &&
            parseInt(table.numero) > parseInt(targetTableNumber)
          )
        ...
        for (let i = 0; i < remainingTablesInLocation.length; i++) {
          ...
          const newTableNumber = String(parseInt(targetTableNumber) + i).padStart(2, "0");
              ...
              <div className="bg-slate-700/50 rounded-lg p-4 space-y-3">
                ...
                >
                  <div className="font-semibold text-orange-400">
                    Opción 2: Mover mesa específica
                  </div>
                  <div className="text-sm text-slate-300 mt-1">
                    Mover la mesa {intelligentOptions.targetNumber} de{" "}
                    ...
                    {intelligentOptions.targetLocation === "adentro"
                      ? "Adentro"
                      : "Afuera"}{" "}
                    y reordenar las mesas restantes
                  </div>
                </button>
              </div>
            </div>
