  ...
  const handleIntelligentOption = async (option) => {
    try {
      if (option === "create-multiple") {
        ...
        // Mover mesa de otra ubicación y reordenar
        const otherLocation = intelligentOptions.targetLocation === "adentro" ? "afuera" : "adentro";
        const otherLocationTables = getTablesByLocation(otherLocation);
        
        if (otherLocationTables.length === 0) {
          alert("No hay mesas disponibles para mover");
          return;
        }
        
        // Ordenar mesas de la otra ubicación
        const sortedOtherTables = otherLocationTables.sort(
          (a, b) => parseInt(a.numero) - parseInt(b.numero)
        );
        
        // Mover la primera mesa de la otra ubicación
        ...
        // Actualizar la mesa que se mueve
        ...
        for (let i = 0; i < remainingTables.length; i++) {
          ...
          const newTableNumber = String(parseInt(tableToMove.numero) + i).padStart(2, "0");
          await updateTable(table.id, { numero: newTableNumber });
        }
        
        // Crear la mesa específica que queríamos
        await createMultipleTables(
              ...
              <div className="bg-slate-700/50 rounded-lg p-4 space-y-3">
                ...
                >
                  <div className="font-semibold text-orange-400">
                    Opción 2: Reordenar mesas existentes
                  </div>
                  <div className="text-sm text-slate-300 mt-1">
