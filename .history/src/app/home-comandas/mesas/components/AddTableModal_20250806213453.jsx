...
function AddTableModal({
  ...
  const handleIntelligentOption = async (option) => {
    try {
      if (option === "create-multiple") {
      ...
      } else if (option === "move-and-reorder") {
        // Buscar la mesa existente en la otra ubicación
        const existingTable = tables.find(
          (table) =>
            table.numero ===
              String(intelligentOptions.targetNumber).padStart(2, "0") &&
            table.lugar === intelligentOptions.otherLocation
        );

        if (!existingTable) {
          alert("No se encontró la mesa para mover");
          return;
        }

        // Mover la mesa existente a la nueva ubicación
        await updateTable(existingTable.id, {
          lugar: intelligentOptions.targetLocation,
        });
        ...
        // Reordenar las mesas de la otra ubicación para que empiecen después de la mesa movida
        for (let i = 0; i < otherLocationTables.length; i++) {
          const table = otherLocationTables[i];
          const newTableNumber = String(parseInt(table.numero) + 1).padStart(
            2,
            "0"
          );
          await updateTable(table.id, { numero: newTableNumber });
        ...
        alert(
          `Se movió la mesa ${intelligentOptions.targetNumber} de ${
            ...
            intelligentOptions.targetLocation === "adentro"
              ? "Adentro"
              : "Afuera"
          } y se reordenaron las mesas restantes`
        );
      } else if (option === "create-next") {
        // Crear la mesa que sigue en la secuencia en la otra ubicación
        ...
        alert(
          `Se creó la mesa ${nextTableNumber} en ${intelligentOptions.otherLocation} (siguiente número disponible)`
        );
      } else if (option === "move-all-up-to") {
        const startNumber = intelligentOptions.lastNumberInLocation + 1;
        const endNumber = intelligentOptions.targetNumber;
        const otherLocation = intelligentOptions.otherLocation;
        const targetLocation = intelligentOptions.targetLocation;

        await moveMultipleTablesToLocation(
          startNumber,
          endNumber,
          otherLocation,
          targetLocation
        ...
        alert(
          `Se movieron ${endNumber - startNumber + 1} mesas de ${
            otherLocation === "adentro" ? "Adentro" : "Afuera"
          } a ${targetLocation === "adentro" ? "Adentro" : "Afuera"}`
        );
      } else if (option === "move-specific") {
        const existingTable = tables.find(
          (table) =>
            table.numero ===
              String(intelligentOptions.targetNumber).padStart(2, "0") &&
                  ...
                  <>
                    ...
                    >
                      ...
                      <div className="text-sm text-slate-300 mt-1">
                        Crear la mesa {intelligentOptions.nextNumberInOtherLocation} en {intelligentOptions.otherLocation === "adentro" ? "Adentro" : "Afuera"} y extender el número de mesas
                      </div>
                    </button>

                    <button
                      onClick={() => handleIntelligentOption("choose-different")}
                    ...
                    >
                      ...
                      <div className="text-sm text-slate-300 mt-1">
                        Cancelar y elegir un número diferente para agregar en {intelligentOptions.targetLocation === "adentro" ? "Adentro" : "Afuera"}
                      </div>
                    </button>

                    <button
                      onClick={() => handleIntelligentOption("specify-quantity")}
                    ...
                    >
                      ...
                      <div className="text-sm text-slate-300 mt-1">
                        Indicar cuántas mesas quieres agregar en {intelligentOptions.targetLocation === "adentro" ? "Adentro" : "Afuera"} (se extenderán automáticamente)
                      </div>
                    </button>
                  </>
                ) : (
