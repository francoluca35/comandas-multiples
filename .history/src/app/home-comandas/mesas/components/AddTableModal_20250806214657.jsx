...
function AddTableModal({
  ...
  const handleQuantitySubmit = async () => {
    try {
      ...
      // Si el usuario quiere crear hasta un número específico (como 06), calcular cuántas mesas faltan
      const targetNumber = intelligentOptions.targetNumber;
      const missingTables = targetNumber - lastNumberInTargetLocation;

      // Usar la cantidad especificada o las mesas faltantes hasta el número objetivo
      const tablesToCreate = Math.min(quantity, missingTables);
      const endNumber = startNumber + tablesToCreate - 1;

      // Crear las mesas en la ubicación objetivo
      await createMultipleTables(
        startNumber,
        endNumber,
        intelligentOptions.targetLocation
      ...
      // Reordenar todas las mesas de la otra ubicación para que empiecen después de las nuevas
      const otherLocation =
      ...
      // Reordenar las mesas de la otra ubicación para que empiecen después de las nuevas creadas
      for (let i = 0; i < otherLocationTables.length; i++) {
        const table = otherLocationTables[i];
        const newTableNumber = String(endNumber + i + 1).padStart(2, "0");
        await updateTable(table.id, { numero: newTableNumber });
      ...
      alert(
        `Se crearon ${tablesToCreate} mesas en ${
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-700/30 rounded-lg p-3">
                <p className="text-sm text-slate-300">
                  <span className="font-semibold text-yellow-400">Contexto:</span> Quieres crear la mesa {intelligentOptions.targetNumber} en{" "}
                  {intelligentOptions.targetLocation === "adentro" ? "Adentro" : "Afuera"}, pero ya existe en{" "}
                  {intelligentOptions.otherLocation === "adentro" ? "Adentro" : "Afuera"}.
                </p>
                <p className="text-sm text-slate-300 mt-1">
                  <span className="font-semibold text-green-400">Resultado:</span> Se crearán las mesas faltantes hasta la {intelligentOptions.targetNumber} y se reordenarán las mesas de{" "}
                  {intelligentOptions.otherLocation === "adentro" ? "Adentro" : "Afuera"}.
                </p>
              </div>
