...
function InventarioContent() {
  ...
  useEffect(() => {
    console.log("🎯 Filtro actual:", { tipoFilter, selectedCategoria, searchTerm });
    console.log("✅ Productos filtrados:", filteredProductos.length);
    console.log("📋 Productos mostrados:", filteredProductos.map(p => p.nombre));
            ...
            <div className="flex justify-center">
              <div className="flex bg-slate-800/50 border border-slate-700/50 rounded-lg p-1">
                {["Todas", "Bebidas", "Comidas"].map((tipo) => (
                  ...
                  >
                    {tipo === "Todas" && "🍽️ Todas"}
                    {tipo === "Bebidas" && "🥤 Bebidas"}
                    {tipo === "Comidas" && "🍽️ Comidas"}
                  </button>
                ))}
              </div>
            </div>
