...
export default function InventarioOptimized() {
  ...
  const filteredItems = useMemo(() => {
    let filtered = searchItems(
      searchQuery,
      selectedType === "todos" ? null : selectedType
    );
    ...
    return [
      {
        value: "todos",
        label: "Todos",
        count: bebidas.length + materiaPrima.length,
      },
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-4">
      ...
      <div className="mb-6">
        ...
        {showStats && (
          <Suspense
            fallback={<OptimizedLoading message="Cargando estadísticas..." />}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredItems.length > 0 ? (
          filteredItems.map((item) => (
            <Suspense
              key={item.id}
              fallback={<OptimizedLoading message="Cargando producto..." />}
            >
              <ProductoCard
                producto={item}
                onEdit={handleEdit}
