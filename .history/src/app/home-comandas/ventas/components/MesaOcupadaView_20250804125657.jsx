...
function MesaOcupadaView({ mesa, onBack, onMesaCobrada }) {
  ...
  const orderData = {
    ...
    monto: 100,
    items: [
      { id: 1, producto: "Adicional Nutella", unidades: 1, precio: 100, total: 100 }
    ]
  };
      ...
      <div className="bg-gray-800 p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <span className="text-white font-bold text-xl">Mesa {mesa?.numero}</span>
            {getIcon("table")}
          </div>

          {/* Close Button */}
          <button onClick={onBack} className="w-8 h-8 bg-gray-700 rounded-full flex items-center justify-center text-white">
            {getIcon("close")}
          </button>
        </div>
          ...
          <div className="bg-gray-700 rounded-lg p-4 mb-4">
            ...
            <div className="border-t border-gray-600 pt-2">
              {orderData.items.map((item) => (
                <div key={item.id} className="grid grid-cols-4 gap-4 text-white">
