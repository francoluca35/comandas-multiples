...
function PedidoView({ mesa, onBack, onMesaOcupada }) {
  ...
  const handleTerminar = async () => {
    ...
    try {
      ...
      // Abrir el modal de cobranza en lugar de volver atrás
  ...
  const handlePaymentComplete = (paymentMethod) => {
    ...
    // Si el pago fue exitoso o la mesa fue liberada, volver atrás
    if (paymentMethod === "liberada" || paymentMethod === "efectivo" || paymentMethod === "mercadopago" || paymentMethod === "tarjeta") {
      onMesaOcupada(mesa);
      onBack();
    }
  };
      ...
      {showCobranzaModal && (
        <CobranzaModal
          orderData={{
            mesa: mesa?.numero,
            mesaId: mesa?.id,
            cliente: mesa?.cliente || clientData.name || "Sin nombre",
