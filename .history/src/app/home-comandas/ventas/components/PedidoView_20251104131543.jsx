...
function PedidoView({ mesa, onBack, onMesaOcupada }) {
  ...
  return (
    <div className="h-[100dvh] flex flex-col bg-[#0f172a] overflow-hidden">
      ...
      <div className="flex-shrink-0 bg-[#1e293b]/90 backdrop-blur-lg border-b border-white/10 shadow-2xl z-20 sticky top-0">
        <div className="p-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button 
                onClick={onBack} 
                className="w-12 h-12 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 text-white/80 hover:text-white active:scale-95"
                title="Volver"
              >
              ...
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-gradient-to-br from-blue-500/80 to-violet-500/80 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <button
                  onClick={() => orderItems.length > 0 && setShowOrderDetails(true)}
                  className="w-12 h-12 flex flex-col items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 active:scale-95"
                  disabled={orderItems.length === 0}
                >
                  <svg className="w-6 h-6 text-white/80" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                  </svg>
                  <span className="text-[10px] text-white/60 mt-1">Pedido</span>
                </button>
                {orderItems.length > 0 && (
                  <div className="absolute -top-1 -right-1 w-6 h-6 bg-gradient-to-br from-pink-500 to-rose-500 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-pink-500/30">
                    {orderItems.length}
                  </div>
                )}
              </div>
              ...
              <button
                ...
                className="w-12 h-12 flex flex-col items-center justify-center rounded-xl bg-gradient-to-br from-violet-500/80 to-purple-500/80 hover:from-violet-600/80 hover:to-purple-600/80 active:scale-95 shadow-lg shadow-purple-500/20"
              >
                <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
                <span className="text-[10px] text-white mt-1">Cliente</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="flex-shrink-0 bg-[#1e293b]/90 border-b border-white/10 sticky top-[72px] z-10 backdrop-blur-lg">
        <div className="p-2">
          <div className="flex flex-col gap-3">
            {/* Categorías principales - Scrollable con botones grandes */}
            <div className="relative">
              <div className="flex gap-2 overflow-x-auto pb-1 hide-scrollbar">
                {mainCategories.map((category) => (
                  <button
                    ...
                    className={`flex flex-col items-center min-w-[80px] p-3 rounded-xl transition-all duration-200 ${
                      selectedMainCategory === category.id
                        ? "bg-gradient-to-br from-blue-500 to-violet-500 text-white shadow-lg shadow-blue-500/25"
                        : "bg-white/5 text-white/70 active:scale-95"
                    }`}
                  >
                    <span className="text-lg mb-1">
                      {category.name === "Comidas" ? "🍽️" : 
                ...
                <button
                  ...
                  className={`flex flex-col items-center min-w-[80px] p-3 rounded-xl transition-all duration-200 ${
                    selectedMainCategory === "promociones"
                      ...
                      : "bg-gradient-to-br from-pink-500/20 to-purple-500/20 text-white/90 active:scale-95"
                  }`}
                >
            ...
            {/* Subcategorías con chips scrollables */}
            {selectedMainCategory !== "promociones" && filteredSubCategories.length > 0 && (
              <div className="relative">
                <div className="flex gap-2 overflow-x-auto pb-1 px-1 hide-scrollbar">
                  <button
                    onClick={() => setSelectedSubCategory("")}
                    className={`h-9 px-4 rounded-full flex-shrink-0 transition-all duration-200 text-sm font-medium flex items-center gap-1.5 ${
                      selectedSubCategory === ""
                  ...
                  {filteredSubCategories.map((subCategory) => (
                    <button
                      ...
                      className={`h-9 px-4 rounded-full flex-shrink-0 transition-all duration-200 text-sm font-medium whitespace-nowrap ${
        ...
        <div className="px-3 py-1">
          <div className="w-full h-1 rounded-full bg-white/5 overflow-hidden">
            <div className="w-1/3 h-full bg-gradient-to-r from-blue-500/50 to-violet-500/50 rounded-full"></div>
          </div>
        </div>
      </div>
  ...
  <div className="flex-1 p-2 sm:p-3 overflow-y-auto bg-gradient-to-b from-[#0f172a] to-[#1e293b]">
    {loading ? (
      <div className="flex items-center justify-center h-full">
        <div className="text-center space-y-3">
          <div className="relative">
            <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl bg-gradient-to-br from-blue-500/20 to-violet-500/20 animate-pulse"></div>
            <div className="absolute inset-0 flex items-center justify-center">
              <svg className="w-6 h-6 sm:w-8 sm:h-8 text-white/30 animate-spin" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
            </div>
          </div>
          <p className="text-white/60 text-sm sm:text-base font-medium">Cargando productos...</p>
        </div>
      </div>
    ) : filteredProducts.length > 0 ? (
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 p-3 pb-32">
        {filteredProducts.map((product) => (
          <button 
            key={product.id} 
            onClick={() => handleProductSelect(product)} 
            className={`group relative rounded-2xl transition-all duration-200 active:scale-95 overflow-hidden ${
          ...
          >
            ...
            {/* Contenido */}
            <div className="relative p-4 h-full flex flex-col min-h-[120px]">
              {/* Nombre del producto con límite de líneas */}
              <div className="font-medium text-sm text-white/90 line-clamp-2 mb-2">
                {product.nombre}
              </div>
              ...
              <div className="mt-auto flex items-end justify-between">
                <div className={`font-bold ${
                  ...
                  <div className="text-xs opacity-75">Precio</div>
                  <div className="text-lg">$ {(typeof product.precio === "number" ? product.precio : parseFloat(product.precio) || 0).toLocaleString()}</div>
                </div>
              ...
              {/* Badge de descuento */}
              {product.descuento > 0 && (
                <div className="absolute top-2 right-2 bg-gradient-to-r from-pink-500 to-rose-500 text-white px-2 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-1">
              ...
              {selectedProduct?.id === product.id && (
                <div className="absolute top-2 left-2 w-5 h-5 bg-white rounded-full flex items-center justify-center shadow-lg">
      ...
      <div className="flex items-center justify-center h-full">
        <div className="text-center space-y-4">
          <div className="w-16 h-16 sm:w-20 sm:h-20 mx-auto relative">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-violet-500/20 to-purple-500/20 rounded-2xl -rotate-6"></div>
            <div className="relative w-full h-full bg-white/5 backdrop-blur-sm rounded-2xl flex items-center justify-center">
          ...
          <div>
            <h3 className="text-white/90 text-lg sm:text-xl font-bold mb-2">No hay productos</h3>
            <p className="text-white/60 text-sm px-4 max-w-sm mx-auto">
              {selectedSubCategory 
