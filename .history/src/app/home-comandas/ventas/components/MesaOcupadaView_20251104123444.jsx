...
function MesaOcupadaView({ mesa, onBack, onMesaCobrada, onMesaActualizada }) {
  ...
  const orderData = {
    ...
    datos_cliente: mesa?.datos_cliente || {}, // Incluir datos completos del cliente
    email: mesa?.datos_cliente?.email || "", // Email del cliente
    whatsapp: mesa?.datos_cliente?.whatsapp || "", // WhatsApp del cliente
  ...
  const handlePaymentComplete = (paymentMethod) => {
    ...
    if (
      ...
      onMesaCobrada(mesa, paymentMethod);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col bg-gray-900 overflow-hidden">
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        {/* Action Bar - Mobile fija abajo */}
        <div className="sm:hidden">
          <div className="fixed inset-x-0 bottom-0 z-30 bg-gradient-to-r from-slate-800 to-slate-700 border-t border-slate-600/50 p-3 flex items-center justify-between gap-2">
            <div className="flex items-center gap-3">
              <div className="text-slate-300 text-xs">Total</div>
              <div className="text-white font-bold text-lg">${orderTotal.toLocaleString()}</div>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={handleAdicionar}
                className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg font-semibold flex items-center gap-1"
              >
                <span>➕</span>
                <span>Agregar</span>
              </button>
          ...
          {/* Espaciador para que el contenido no quede detrás de la barra fija */}
          <div className="h-16" />
        </div>
      </div>
