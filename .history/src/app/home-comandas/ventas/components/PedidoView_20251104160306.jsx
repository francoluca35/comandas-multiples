...
import { useSidebar } from "../../home/components/Sidebar";
...
function PedidoView({ mesa, onBack, onPedidoEnviado }) {
  ...
  // Estados principales
  // Inicializar en "todos" para que la vista siempre empiece mostrando todos los productos
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      // Match tailwind breakpoints used in the grid
      if (w >= 1280) setColumnCount(7); // xl
      else if (w >= 1024) setColumnCount(5); // lg
      else if (w >= 768) setColumnCount(4); // md
      else if (w >= 640) setColumnCount(3); // sm
      else setColumnCount(2); // default

      // measure list height
      if (productsContainerRef.current) {
        setListHeight(productsContainerRef.current.clientHeight);
      } else {
  ...
  // Update list height if container height changes (layout)
  useLayoutEffect(() => {
    if (productsContainerRef.current) {
      setListHeight(productsContainerRef.current.clientHeight);
    }
  ...
  useEffect(() => {
    // No solicitar subcategorías cuando la selección sea "todos"
    if (selectedMainCategory && selectedMainCategory !== "todos") {
      fetchSubCategories(selectedMainCategory);
    }
  }, [selectedMainCategory]);

  // Filtrar productos según categoría seleccionada (memoizado para rendimiento)
  const filteredProducts = useMemo(() => {
    if (selectedMainCategory === "todos") return products;

    // Cuando hay subcategoría seleccionada, filtrar por ambas
  ...
  const ProductCard = React.memo(({ product, expanded, productQuantity, onSelect, onAdd }) => {
    if (expanded) {
      return (
        <div className="bg-blue-600 rounded-lg p-4 border-2 border-blue-400">
          <div className="text-white font-bold mb-2 text-sm">{product.nombre}</div>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-white font-bold text-lg">{productQuantity}</span>
            <button
          ...
          <button onClick={onAdd} className="w-full bg-green-500 hover:bg-green-600 text-white py-2 rounded-lg font-semibold text-sm">
            Añadir producto
          </button>
        </div>
      );
    ...
    return (
      <div onClick={onSelect} className="bg-gray-700 rounded-lg p-4 cursor-pointer hover:border-2 hover:border-blue-500 transition-all min-h-[100px]">
        <div className="text-white text-sm font-medium truncate">{product.nombre}</div>
        <div className="text-blue-400 text-xs mt-1">${((product.precio * (1 - (product.descuento || 0) / 100))).toFixed(2)}</div>
      </div>
    );
  });
  ...
  const handleAddProduct = (product) => {
    ...
    if (existingProduct) {
      ...
      const precioFinal = product.precio * (1 - (product.descuento || 0) / 100);
      setSelectedProducts((prev) => [
        ...prev,
        {
          ...product,
    ...
    return (
      <div className="h-full flex flex-col bg-gray-900 overflow-hidden">
        ...
        <div className="flex-1 p-6 overflow-y-auto">
          {/* Virtualized list container - use full width so react-window rows can create their own grid */}
          <div className="w-full">
            {loading ? (
              ...
              // Virtualized list: render rows where each row contains columnCount items
              <div ref={productsContainerRef} style={{ height: '100%' }}>
                <List
                  height={listHeight}
                  itemCount={Math.ceil(filteredProducts.length / columnCount)}
                  itemSize={160} /* row height */
                  width="100%"
                >
                  {({ index, style }) => {
                    const start = index * columnCount;
                    const rowItems = filteredProducts.slice(start, start + columnCount);
                    return (
                      <div
                        style={{
                          ...style,
        ...
        <div
          className={`fixed bottom-0 ${
            isExpanded
              ? "left-56 sm:left-64 md:left-72 lg:left-80 xl:left-96"
              : "left-16 sm:left-20"
          } right-0 bg-gray-800 border-t border-gray-700 p-4 z-50`}
        >
          <div className="flex items-center justify-between max-w-7xl mx-auto">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="bg-green-600 hover:bg-green-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white px-6 py-3 rounded-lg font-bold"
              >
                Enviar a cocina
              </button>
            </div>
      ...
      <div
        className={`fixed bottom-0 ${
          isExpanded
            ? "left-56 sm:left-64 md:left-72 lg:left-80 xl:left-96"
            : "left-16 sm:left-20"
        } right-0 bg-gray-800 border-t border-blue-600 p-4 z-20`}
      >
