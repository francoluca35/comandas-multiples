...
function MesasView({ onMesaClick }) {
  ...
  const handleMesaClick = (mesa) => {
    ...
    // Simular historial de pedidos (en un caso real vendría de la base de datos)
    if (mesa.estado === "ocupado") {
      setOrderHistory([
        { id: 1, producto: "Hamburguesa", cantidad: 2, precio: 1500, total: 3000 },
        { id: 2, producto: "Coca Cola", cantidad: 1, precio: 500, total: 500 },
        { id: 3, producto: "Papas Fritas", cantidad: 1, precio: 800, total: 800 },
      ]);
    } else {
      setOrderHistory([]);
    }
      ...
      {selectedMesa && (
        <div className="w-80 bg-gray-800 border-l border-gray-700 p-6">
          ...
          <div className="bg-gray-700 rounded-lg p-4 mb-6">
            <h4 className="text-white font-medium mb-3">Cantidad de Producto</h4>
          ...
          <div className="bg-gray-700 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-white font-medium">Historial del Pedido</h4>
              <button
                onClick={() => setShowOrderHistory(!showOrderHistory)}
                className="text-blue-400 hover:text-blue-300 text-sm"
              >
                {showOrderHistory ? "Ocultar" : "Ver más"}
              </button>
            </div>
                ...
                {orderHistory.length > 0 ? (
                  orderHistory.map((item) => (
                    <div key={item.id} className="bg-gray-600 rounded p-2">
                      <div className="text-white text-sm font-medium">{item.producto}</div>
              ...
              <div className="text-gray-400 text-sm">
                {orderHistory.length > 0 ? (
                  <div className="text-center py-2">
                    {orderHistory.length} productos agregados
                  </div>
                ) : (
                  <div className="text-center py-2">
                    Sin productos
