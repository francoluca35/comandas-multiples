...
function MesasCroquisView({ onMesaClick }) {
  ...
  const calculateResponsivePositions = (tables, containerRef) => {
    if (!containerRef.current || tables.length === 0) return { positions: {}, savedPositions: {}, sizes: {} };
    ...
    tables.forEach(mesa => {
      ...
      if (mesa.position) {
        ...
        let defaultPos;
        const margen = 20;
        const espacioDisponibleX = containerWidth - (margen * 2);
        const espacioDisponibleY = containerHeight - (margen * 2);
        
        if (mesa.lugar === "adentro") {
          // Mesas adentro en la parte superior
          const mitadY = hasAfuera ? containerHeight / 2 : containerHeight;
          const espacioY = mitadY - (margen * 2);
          
          // Distribución en grid responsivo
          const columnas = Math.min(3, Math.floor(espacioDisponibleX / 120)); // Máximo 3 columnas
          const filas = Math.ceil(mesasAdentro.length / columnas);
          const index = mesasAdentro.findIndex(m => m.id === mesa.id);
          
          const col = index % columnas;
          const fila = Math.floor(index / columnas);
          
          defaultPos = { 
            x: margen + (col * (espacioDisponibleX / columnas)) + Math.random() * 20,
            y: margen + (fila * (espacioY / Math.max(filas, 1))) + Math.random() * 20
          };
        } else if (mesa.lugar === "afuera") {
          // Mesas afuera en la parte inferior
          const mitadY = containerHeight / 2;
          const espacioY = mitadY - (margen * 2);
          
          // Distribución en grid responsivo
          const columnas = Math.min(3, Math.floor(espacioDisponibleX / 120));
          const filas = Math.ceil(mesasAfuera.length / columnas);
          const index = mesasAfuera.findIndex(m => m.id === mesa.id);
          ...
          defaultPos = { 
            x: margen + (col * (espacioDisponibleX / columnas)) + Math.random() * 20,
            y: mitadY + margen + (fila * (espacioY / Math.max(filas, 1))) + Math.random() * 20
          };
        } else {
          // Fallback para mesas sin ubicación
          defaultPos = { 
            x: margen + Math.random() * (espacioDisponibleX / 2), 
            y: margen + Math.random() * (espacioDisponibleY / 2) 
          };
        }
    ...
    return { positions, savedPositions, sizes };
  };

  // Cargar posiciones guardadas de las mesas
  useEffect(() => {
    if (tables.length > 0) {
      const { positions, savedPositions, sizes } = calculateResponsivePositions(tables, containerRef);
  ...
  // Recalcular posiciones cuando cambie el tamaño del contenedor
  useEffect(() => {
    const handleResize = () => {
      if (tables.length > 0) {
        const { positions, savedPositions, sizes } = calculateResponsivePositions(tables, containerRef);
  ...
  const checkCollision = (mesaId, newX, newY) => {
    ...
    if (hasAfuera) {
      ...
      const margen = 30; // Margen reducido para mejor responsividad
  ...
  return (
    <div className="h-full bg-[#1a1a1a] p-3 sm:p-4 md:p-6 lg:p-8 max-w-full overflow-hidden">
        ...
        {viewMode === "croquis" ? (
          editMode ? (
            ...
            <p className="text-gray-400 text-sm">
              🏪 Vista croquis: Haz click en "Editar Layout" para reorganizar las mesas
            </p>
          )
        ) : (
          <p className="text-gray-400 text-sm">
            📋 Vista grid: Haz click en una mesa para seleccionarla
          </p>
        )}
      </div>
      ...
      {viewMode === "croquis" && (
        <div 
          ref={containerRef}
          className={`relative w-full h-[calc(100vh-200px)] sm:h-[calc(100vh-180px)] md:h-[calc(100vh-160px)] lg:h-[calc(100vh-140px)] bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800 rounded-2xl border-2 overflow-hidden ${
        ...
        {/* Línea divisoria si hay mesas afuera - Vertical para tablet */}
        {hasAfuera && (
          <div className="absolute left-0 right-0 top-1/2 h-0.5 bg-gradient-to-r from-transparent via-slate-500 to-transparent transform -translate-y-1/2 z-10"></div>
        )}

        {/* Etiquetas de sección - Centradas horizontalmente */}
        <div className="absolute top-2 left-1/2 transform -translate-x-1/2 z-20">
          <div className="bg-green-600/90 backdrop-blur-sm text-white px-6 py-2 rounded-lg shadow-lg border-2 border-green-400">
        ...
        {hasAfuera && (
          <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 z-20">
            <div className="bg-orange-600/90 backdrop-blur-sm text-white px-6 py-2 rounded-lg shadow-lg border-2 border-orange-400">
