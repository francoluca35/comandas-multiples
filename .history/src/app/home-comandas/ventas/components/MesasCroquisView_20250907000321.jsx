...
function MesasCroquisView({ onMesaClick }) {
  ...
  const handleMouseMove = (e) => {
    ...
    // Aplicar límites de sección si hay mesas afuera
    if (hasAfuera) {
      if (draggedMesa.lugar === "adentro") {
        clampedX = Math.min(clampedX, 200); // Límite izquierdo para mesas adentro
      } else if (draggedMesa.lugar === "afuera") {
        clampedX = Math.max(clampedX, 250); // Límite derecho para mesas afuera
      }
    }
