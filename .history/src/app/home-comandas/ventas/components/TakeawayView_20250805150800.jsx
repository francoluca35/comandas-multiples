...
function TakeawayView({ onBack }) {
  const { products, mainCategories, subCategories, fetchMainCategories, fetchSubCategories, fetchAllProducts } = useProducts();
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setClientData(prev => ({ ...prev, mPago: e.target.value }))}
            ...
            {selectedProducts.length === 0 ? (
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {selectedProducts.map((product) => (
                  <div key={product.id} className="flex items-center justify-between bg-slate-600 rounded-lg p-3">
                    <div className="flex-1">
                      <h4 className="text-white font-medium">{product.nombre}</h4>
                      <p className="text-gray-300 text-sm">
                        ${(product.precio * (1 - product.descuento / 100)).toFixed(2)}
                        {product.descuento > 0 && (
                          <span className="text-red-400 ml-2">-{product.descuento}%</span>
                        )}
                      </p>
                    </div>
        ...
        <div className="grid grid-cols-2 gap-4 max-h-96 overflow-y-auto">
          {filteredProducts.map((product) => (
            ...
            >
              ...
              <p className="text-gray-400 text-sm mb-2">{product.descripcion}</p>
