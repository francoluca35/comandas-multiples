...
function CobranzaModal({ isOpen, onClose, orderData, onPaymentComplete }) {
  ...
  const handleLiberarMesa = async () => {
    if (!confirm("¿Estás seguro de que quieres liberar la mesa? Esta acción eliminará todos los datos del pedido.")) {
      return;
    }

    try {
      // Necesitamos el ID de la mesa para actualizarla
      // Asumiendo que orderData.mesa contiene el número de mesa, necesitamos encontrar el ID
      // Por ahora, vamos a usar una lógica simple basada en el número de mesa
      
      console.log("Liberando mesa:", orderData.mesa);
      
      // Aquí necesitaríamos obtener el ID de la mesa basado en el número
      // Por ahora, vamos a simular la liberación
      ...
      <div className="bg-gray-800 rounded-lg p-6 w-96 max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-gray-700 rounded-lg p-3 text-center">
            ...
            <div className="text-white text-sm">Proforma</div>
            <div className="text-white font-bold">
              {orderData?.proforma || "003"}
            </div>
          </div>
          ...
          <div className="bg-gray-700 rounded-lg p-3 text-center">
            ...
            <div className="text-white text-sm">Monto</div>
            <div className="text-white font-bold">
              $ {orderData?.monto || "100"}
            </div>
          </div>
        ...
        <button
          ...
          className={`w-full rounded-lg px-3 py-2 flex items-center justify-center space-x-2 ${
            selectedPaymentMethod
