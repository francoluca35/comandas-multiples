...
function MesasCroquisView({ onMesaClick }) {
      ...
      tables.forEach(mesa => {
        ...
        if (mesa.position) {
          ...
          if (mesa.lugar === "adentro") {
            // Mesas adentro en el lado izquierdo
            defaultPos = { 
              x: Math.random() * (hasAfuera ? 200 : 400), 
              y: Math.random() * 300 
            };
          } else if (mesa.lugar === "afuera") {
            // Mesas afuera en el lado derecho
            defaultPos = { 
  ...
  const checkCollision = (mesaId, newX, newY) => {
    ...
    // Verificar límites de sección - más flexible
    if (hasAfuera) {
      // Permitir que las mesas adentro se muevan más hacia la derecha
      if (mesa.lugar === "adentro" && newX > 350) {
        return true; // Mesa adentro no puede ir demasiado a la sección afuera
      }
      if (mesa.lugar === "afuera" && newX < 200) {
  ...
  const handleMouseMove = (e) => {
    ...
    // Aplicar límites de sección si hay mesas afuera - más flexible
    if (hasAfuera) {
      if (draggedMesa.lugar === "adentro") {
        clampedX = Math.min(clampedX, 350); // Permitir más movimiento hacia la derecha
      } else if (draggedMesa.lugar === "afuera") {
        clampedX = Math.max(clampedX, 200); // Límite derecho para mesas afuera
      }
    }
  ...
  return (
    <div className="h-full bg-[#1a1a1a] p-3 sm:p-4 md:p-6 lg:p-8 max-w-full overflow-hidden">
      ...
      >
        ...
        {hasAfuera && (
          <div className="absolute top-0 bottom-0 left-1/3 w-0.5 bg-gradient-to-b from-transparent via-slate-500 to-transparent transform -translate-x-1/2 z-10"></div>
        )}
