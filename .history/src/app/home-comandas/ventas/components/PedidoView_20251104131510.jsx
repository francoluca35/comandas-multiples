...
function PedidoView({ mesa, onBack, onMesaOcupada }) {
  ...
  <div className="flex-1 p-2 sm:p-3 overflow-y-auto bg-gradient-to-b from-[#0f172a] to-[#1e293b]">
    {loading ? (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-2 sm:gap-3 pb-20">
        {filteredProducts.map((product) => (
          <button 
            key={product.id} 
            onClick={() => handleProductSelect(product)} 
            className={`group relative rounded-xl transition-all duration-300 hover:scale-102 active:scale-98 overflow-hidden ${
          ...
          >
            ...
            <div className={`absolute inset-0 ${
              selectedProduct?.id === product.id
                ? "bg-gradient-to-br from-blue-500/90 to-violet-500/90"
                : "bg-white/5 backdrop-blur-sm"
            } transition-all duration-300 group-hover:bg-white/10`}></div>

            {/* Contenido */}
            <div className="relative p-3 h-full flex flex-col">
              {/* Nombre del producto con límite de líneas */}
              <div className="font-medium text-xs sm:text-sm text-white/90 line-clamp-2 mb-2">
                {product.nombre}
              </div>
              ...
              {/* Badge de descuento */}
              {product.descuento > 0 && (
                <div className="absolute top-1 right-1 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold shadow-lg">
                  -{product.descuento}%
                </div>
              )}
