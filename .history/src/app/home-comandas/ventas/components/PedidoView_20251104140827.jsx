...
function PedidoView({ mesa, onBack, onMesaOcupada }) {
  ...
  return (
    <div className="h-screen max-h-screen min-h-screen flex flex-col bg-[#1A2036] overflow-hidden">
      {/* Header */}
      <div className="flex-shrink-0 bg-[#363B4D] border-b border-slate-700/50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-xl font-bold text-white">Mesa {mesa?.numero}</h1>
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-3 py-2 rounded-lg font-medium transition-all duration-200 shadow-lg"
              >
              ...
              {/* Botones de Acción cuando hay items - Solo en desktop */}
              {orderItems.length > 0 && (
                <>
                  <button
                    onClick={() => setShowOrderDetails(true)}
                    className="hidden md:flex bg-[#D98C00] hover:bg-[#C77A00] text-white px-4 py-2 rounded-lg font-bold transition-all shadow-lg"
                  >
                    ver items
                  </button>
                  <button
                    onClick={handleTerminar}
                    className="hidden md:flex bg-[#00A884] hover:bg-[#009673] text-white px-4 py-2 rounded-lg font-bold transition-all shadow-lg"
                  >
                    enviar a cocina
                  </button>
                </>
      ...
      <div className="flex-shrink-0 bg-slate-800/50 border-b border-slate-700/50">
        <div className="p-2 sm:p-3">
          ...
          <div className="flex space-x-2 overflow-x-auto mb-2 pb-1">
            {mainCategories.map((category) => (
              <button
                key={category.id}
                onClick={() => setSelectedMainCategory(category.id)}
                className={`px-3 py-2 rounded-lg flex-shrink-0 transition-all duration-200 font-medium text-sm ${
          ...
          {selectedMainCategory !== "promociones" && (
            <div className="flex space-x-2 overflow-x-auto">
              <button
                ...
                className={`px-2 py-1 rounded-lg flex-shrink-0 transition-all duration-200 text-xs font-medium ${
                  selectedSubCategory === ""
                    ? "bg-gradient-to-r from-emerald-500 to-emerald-600 text-white"
                    : "bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white"
                }`}
              >
                {selectedMainCategory === "comida" ? "🍽️ Todas" : selectedMainCategory === "bebidas" ? "🥤 Todas" : "🍽️ Todos"}
              </button>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="w-full lg:w-1/3 bg-[#1A2036] border-r border-slate-700/50 overflow-y-auto">
          <div className="h-full flex flex-col p-4">
            {selectedProduct ? (
              <>
                {/* Información del Producto */}
                <div className="mb-4">
                  <h3 className="text-white font-bold text-lg mb-4">{selectedProduct.nombre}</h3>
                  ...
                  <div className="flex items-center justify-center space-x-2 mb-4">
                    <button 
                      ...
                      className="w-8 h-8 bg-slate-700 hover:bg-slate-600 disabled:bg-slate-600 disabled:cursor-not-allowed rounded-full flex items-center justify-center text-white transition-all"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" />
                      </svg>
                    </button>
                    <div className="bg-white text-slate-800 rounded-lg px-4 py-2 font-bold">{productQuantity} x</div>
                    <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-lg px-4 py-2 font-bold">$ {selectedProduct.precio.toLocaleString()}</div>
                    <button 
                      onClick={() => handleQuantityChange(productQuantity + 1)} 
                      className="w-8 h-8 bg-emerald-500 hover:bg-emerald-600 rounded-full flex items-center justify-center text-white transition-all"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {/* Total del Item */}
                  <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg p-3 mb-4">
                    <div className="text-white text-center">
                      <div className="text-xs opacity-90 mb-1">Total del Item</div>
                      <div className="text-xl font-bold">$ {(selectedProduct.precio * productQuantity).toLocaleString()}</div>
                    </div>
                  </div>
                  ...
                  <div className="mb-4 p-3 bg-slate-700/50 rounded-lg">
                    ...
                    <div className="text-sm text-white">{selectedProduct.descripcion || "bebidas"}</div>
                  </div>
                  
                  {/* Botón Nota */}
                  <button 
                    onClick={() => setShowNotaModal(true)} 
                    className="w-full bg-slate-600 hover:bg-slate-500 text-white rounded-lg px-3 py-2 flex items-center justify-center space-x-2 transition-all mb-4"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button 
                  ...
                  className="w-full bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white rounded-xl px-4 py-3 flex items-center justify-center space-x-2 font-bold transition-all mb-4"
                >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                  </svg>
        ...
        <div className={`flex-1 bg-[#1A2036] overflow-y-auto p-4 ${orderItems.length > 0 ? 'pb-32 md:pb-4' : ''}`}>
          {loading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {filteredProducts.map((product) => (
                <div 
                  ...
                  className={`bg-slate-800 rounded-xl p-3 cursor-pointer transition-all hover:bg-slate-700 ${
                    selectedProduct?.id === product.id ? 'ring-2 ring-orange-500' : ''
                  }`}
                >
                  <div className="text-white font-semibold text-sm mb-2 truncate">{product.nombre}</div>
                  <div className="text-emerald-400 font-bold">$ {(typeof product.precio === "number" ? product.precio : parseFloat(product.precio) || 0).toLocaleString()}</div>
                </div>
              ))}
            </div>
          ) : (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <h3 className="text-white text-lg font-bold mb-2">No hay productos</h3>
                <p className="text-slate-400 text-sm">Selecciona una categoría para ver los productos disponibles</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Barra fija inferior para mobile cuando hay items */}
      {orderItems.length > 0 && (
        <div className="md:hidden fixed inset-x-0 bottom-0 z-50 bg-[#1A2036] border-t-2 border-slate-700/60 shadow-2xl">
          <div className="p-4 space-y-3">
            {/* Total del pedido y historial */}
            <div>
              <div className="text-white text-base font-semibold mb-1">Total del pedido: ${orderTotal.toLocaleString()}</div>
              <button className="text-blue-400 text-sm hover:text-blue-300 underline font-medium">
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowOrderDetails(true)}
                className="flex-1 bg-[#D98C00] hover:bg-[#C77A00] active:bg-[#B87900] text-white rounded-lg px-4 py-3.5 font-bold text-base transition-all shadow-lg"
              >
                ver pedido
              </button>
              <button
                onClick={handleTerminar}
                className="flex-1 bg-[#00A884] hover:bg-[#009673] active:bg-[#008562] text-white rounded-lg px-4 py-3.5 font-bold text-base transition-all shadow-lg"
              >
                enviar a cocina
              </button>
            </div>
