...
function NuevoAgregarPedido({ mesa, onBack, onPedidoAgregado }) {
  ...
  const handleEnviarACocina = async () => {
    ...
    try {
      console.log("🚀 Enviando pedido adicional a cocina...");
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-6xl mx-4 max-h-[90vh] overflow-hidden">
        ...
        <div className="bg-gray-800 text-white p-4 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold">Agregar Pedido - Mesa {mesa?.numero}</h2>
            <p className="text-gray-300">Cliente: {mesa?.cliente || "Sin nombre"}</p>
            <p className="text-sm text-gray-400">Total actual: ${(mesa?.total || 0).toLocaleString()}</p>
          </div>
          <button
            ...
            <div className="border-t pt-4 space-y-2">
              ...
              {showSuccess && (
                <div className="mt-4 p-3 bg-green-100 border border-green-400 text-green-700 rounded-lg text-center">
                  ...
                  <div className="text-sm">Los productos se han agregado a la mesa</div>
                </div>
              )}
            </div>
          </div>
