...
function MesasCroquisView({ onMesaClick }) {
      ...
      tables.forEach(mesa => {
        ...
        if (mesa.position) {
          ...
          if (mesa.lugar === "adentro") {
            ...
            const mitad = hasAfuera ? 300 : 600; // 300px si hay afuera, 600px si no
            defaultPos = { 
              x: Math.random() * 600, // Ancho completo
              y: Math.random() * mitad 
            };
            ...
            defaultPos = { 
              x: Math.random() * 600, // Ancho completo
              y: 300 + Math.random() * 300 // Empieza en la mitad vertical
            };
          } else {
  ...
  const checkCollision = (mesaId, newX, newY) => {
    ...
    if (hasAfuera) {
      ...
      if (mesa.lugar === "adentro" && newY > mitad) {
        return true; // Mesa adentro no puede ir a la sección afuera (abajo)
      }
      if (mesa.lugar === "afuera" && newY < mitad) {
  ...
  const handleMouseMove = (e) => {
    ...
    if (hasAfuera) {
      ...
      if (draggedMesa.lugar === "adentro") {
        clampedY = Math.min(clampedY, mitad); // Límite en la mitad vertical para mesas adentro
      } else if (draggedMesa.lugar === "afuera") {
        clampedY = Math.max(clampedY, mitad); // Límite en la mitad vertical para mesas afuera
      }
    }
