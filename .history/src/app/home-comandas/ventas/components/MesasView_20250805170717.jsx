...
function MesasView({ onMesaClick }) {
  ...
  const handleEliminarComandas = async (mesa, e) => {
    ...
    if (!confirm(`¿Estás seguro de que quieres eliminar las comandas de la Mesa ${mesa.numero}? Esta acción no se puede deshacer.`)) {
      return;
    }

    try {
      // Actualizar la mesa en Firestore para liberarla
      const mesaRef = doc(db, `restaurantes/${getRestaurantId()}/tables/${mesa.id}`);
