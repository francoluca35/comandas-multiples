  ...
  const handleFinalizarPedido = async () => {
    ...
    try {
      ...
      try {
        ...
        const docRef = await addDoc(pedidosFinalizadosRef, pedidoFinalizado);
         ...
         <div className="flex-shrink-0 bg-gradient-to-r from-slate-900 to-slate-800 border-t border-slate-600/50 shadow-xl">
           <div className="p-3 sm:p-4">
             ...
             <button
               ...
               className="w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl px-4 py-3 sm:px-6 sm:py-4 flex items-center justify-center space-x-2 font-bold text-base sm:text-lg transition-all duration-200 transform hover:scale-105 shadow-xl"
             >
               <svg className="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
               </svg>
               <span>Finalizar Pedido - $ {orderTotal.toLocaleString()}</span>
             </button>
           </div>
         </div>
       )}
