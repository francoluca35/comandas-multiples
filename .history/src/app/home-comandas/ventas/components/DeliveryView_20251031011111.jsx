...
export default function DeliveryView() {
  ...
  // Obtener subcategorías dinámicamente de los productos
  const getSubCategories = (mainCategoryId) => {
    if (!products || products.length === 0) return [];
    
    const categoryProducts = products.filter(
      (product) => product.mainCategoryId === mainCategoryId || product.categoria === mainCategoryId
    );
    
    const uniqueSubCategories = [...new Set(
      categoryProducts
        .map(product => product.subCategoryId || product.subcategoria)
        .filter(Boolean)
    )];
    
    // Para bebidas, usar las subcategorías exactas del inventario
    if (mainCategoryId === "bebidas") {
      return [
        { id: "sin alcohol", name: "Sin Alcohol" },
        { id: "con alcohol", name: "Con Alcohol" }
      ];
    }
    
    return uniqueSubCategories.map(subCat => ({
      id: subCat,
      name: subCat === "sin alcohol" ? "Sin Alcohol" : 
            subCat === "con alcohol" ? "Con Alcohol" :
    ...
    <div className="h-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex flex-col">
      ...
      {showMenu && (
        <div className="flex-1 bg-gradient-to-br from-slate-800/90 to-slate-900/90 backdrop-blur-sm flex flex-col shadow-2xl">
          ...
          {/* Grid de Productos */}
          <div className="flex-1 p-4 overflow-y-auto pb-24" style={{ 
            ...
            <div className="max-w-3xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2 md:col-span-2">
                  <label className="text-sm font-semibold text-slate-200 flex items-center space-x-2">
                    <div className="p-1.5 bg-gradient-to-br from-purple-500/20 to-purple-600/20 rounded">
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    ...
                    className="w-full px-3 py-2 bg-slate-700/80 border border-slate-600/50 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 resize-none text-sm transition-all duration-200 hover:bg-slate-700/90"
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div>
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white py-3 px-4 rounded-lg font-semibold transition-all duration-300 text-sm shadow-lg hover:shadow-xl transform hover:scale-105 flex items-center justify-center space-x-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                  </svg>
      ...
      {/* Barra de productos seleccionados - FIJA desde el sidebar - SCROLLEABLE - Solo visible en Seleccionar Productos */}
      {selectedProducts.length > 0 && showMenu && (
        <div className="fixed bottom-0 left-16 sm:left-20 right-0 z-[100] bg-gradient-to-r from-blue-600/98 to-blue-700/98 border-t-2 border-blue-400 backdrop-blur-md shadow-2xl">
          {/* Header desplegable */}
          <div 
            className="p-3 cursor-pointer hover:bg-blue-600/80 transition-colors duration-200 bg-slate-900/98"
          ...
          {/* Lista desplegable de productos - SCROLLEABLE */}
          {showSelectedProducts && (
            <div className="border-t border-blue-500/30 bg-slate-900/98 max-h-[50vh] overflow-y-auto shadow-lg">
              {selectedProducts.map((product) => (
                <div key={product.id} className="p-3 border-b border-slate-700/30 last:border-b-0 hover:bg-slate-800/50 transition-colors">
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0">
                      <h4 className="text-white font-semibold text-sm mb-1 truncate">{product.nombre}</h4>
