...
function MesasCroquisView({ onMesaClick }) {
  ...
  const checkCollision = (mesaId, newX, newY) => {
    ...
    // Verificar límites de sección
    if (hasAfuera) {
      if (mesa.lugar === "adentro" && newX > 200) {
        return true; // Mesa adentro no puede ir a la sección afuera
      }
      if (mesa.lugar === "afuera" && newX < 250) {
      ...
      <div className="text-center mb-4">
        {editMode ? (
          <p className="text-orange-400 text-sm font-medium">
            ✏️ Modo edición: Arrastra las mesas para organizarlas. Las mesas no pueden superponerse ni cambiar de sección.
          </p>
        ) : (
