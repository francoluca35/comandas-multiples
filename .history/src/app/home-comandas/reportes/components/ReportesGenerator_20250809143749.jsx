...
export default function ReportesGenerator() {
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      link.href = '#';
      link.download = `reporte_${selectedReport.id}_${new Date().toISOString().split('T')[0]}.xlsx`;
      link.click();
  ...
  const getReportFilters = () => {
    ...
    const filterConfigs = {
      ventas: [
        { key: "tipoVenta", label: "Tipo de Venta", type: "select", options: ["Mesa", "Delivery", "Takeaway"] },
      ...
      financiero: [
        { key: "tipoMovimiento", label: "Tipo de Movimiento", type: "select", options: ["Ingreso", "Egreso"] },
        { key: "categoria", label: "Categoría", type: "text" },
        { key: "empleado", label: "Empleado", type: "text" }
      ],
      empleados: [
        ...
        { key: "activo", label: "Activo", type: "select", options: ["Sí", "No"] },
        { key: "fechaContratacion", label: "Fecha de Contratación", type: "date" }
      ],
      mesas: [
      ...
      inventario: [
        ...
        { key: "stockBajo", label: "Stock Bajo", type: "select", options: ["Sí", "No"] }
      ]
    };
      ...
      {!selectedReport && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-500">
            Elige el tipo de reporte que deseas generar y configura los parámetros necesarios
          </p>
        </div>
      )}
    </div>
