...
function Login() {
  ...
  const crearUsuario = async () => {
    try {
      const nombreResto = localStorage.getItem("nombreResto");
      if (!nombreResto || !nuevoUsuario.usuario || !nuevoUsuario.password) {
      ...
      if (usuarios.length >= restauranteInfo.cantUsuarios) {
        Swal.fire("Error", `Solo puedes crear ${restauranteInfo.cantUsuarios} usuarios`, "error");
        return;
      }
      ...
      // Crear usuario en el restaurante específico
      const usersRef = collection(db, `restaurantes/${restauranteId}/users`);
  ...
  const handleLogin = async () => {
    ...
    if (password === usuarioSeleccionado.password) {
      ...
      await Swal.fire({
        ...
        imageUrl: usuarioSeleccionado.imagen || restauranteInfo?.logo || "/Assets/LogoApp.png",
        imageWidth: 100,
        imageHeight: 100,
          ...
          <div className="bg-[#2e2e2e] p-4 rounded mb-4">
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                onChange={(e) => setNuevoUsuario({...nuevoUsuario, imagen: e.target.value})}
                className="w-full p-2 rounded text-black"
              />
