...
const PedidoCard = ({ pedido, onStatusChange, tipo }) => {
  ...
  // Función para agrupar productos duplicados por nombre
  const agruparProductos = (productos) => {
    ...
    productos.forEach((item) => {
      ...
      const cantidad = parseInt(item.cantidad || item.unidades || 1);
      ...
      if (productosAgrupados[nombreProducto]) {
        ...
        // Mantener otros datos del primer producto encontrado
      } else {
  ...
  return (
    <div className={`backdrop-blur-sm rounded-xl p-5 border shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02] ${
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">
            {agruparProductos(pedido.productos)?.length || 0} items
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2 mb-4">
        {agruparProductos(pedido.productos)?.map((item, index) => (
          <div
            key={index}
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-2 flex-1">
                <span className={`text-xs font-bold px-2 py-1 rounded-full min-w-[24px] text-center mt-0.5 ${
                  isTakeaway 
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-gray-700 font-medium">{item.nombre || item.producto}</div>
            ...
            {(item.adicionales || (item.adicional && item.adicional.length > 0)) && (
              <div className={`mt-2 pt-2 border-t ${
                ...
                <div className="flex flex-wrap gap-1">
                  {(item.adicionales || item.adicional || []).map((adicional, idx) => (
                    <span
                      key={idx}
                    ...
                    >
                      {typeof adicional === 'string' ? adicional : (adicional.nombre || adicional.name || adicional)}
                    </span>
                  ))}
                </div>
              </div>
      ...
      <div className="flex justify-end">
        {tipo === "primeros" ? (
          ...
          <button
            onClick={() => onStatusChange(pedido.id, "eliminar")}
...
function CocinaContent() {
  ...
  const handleStatusChange = async (pedidoId, nuevoEstado) => {
    try {
      if (nuevoEstado === "listo") {
        ...
        if (pedido) {
          // Notificar que el pedido está listo (sin sonido, solo visual)
          notifyOrderReady(pedido, false);
        }
  ...
  const pedidosEnPreparacion = getPedidosEnPreparacion();
  const pedidosListos = getPedidosListos();
  
  // Combinar pedidos pendientes y en preparación, evitando duplicados
  const pedidosPrimeros = [
    ...pedidosPendientes,
    ...pedidosEnPreparacion,
  ];
  
  // Filtrar pedidos listos para evitar duplicados con los primeros
  const pedidosHechos = pedidosListos.filter(pedidoListo => 
    !pedidosPrimeros.some(pedidoPrimero => pedidoPrimero.id === pedidoListo.id)
  );
            ...
            <div className="max-w-6xl mx-auto">
                    ...
                    {pedidosHechos.length > 0 ? (
                      ...
                      <div className="col-span-full bg-white/70 rounded-xl p-8 border border-green-200 shadow-sm">
                        <div className="text-center">
                          ...
                          <p className="text-green-500 text-sm mt-1">Los pedidos marcados como listos aparecerán aquí</p>
                        </div>
                      </div>
                    )}
                  </div>
              ...
              <div className="mt-8 text-center bg-gradient-to-r from-slate-700/50 to-slate-800/50 rounded-2xl p-6 border border-slate-600/50 backdrop-blur-sm">
                ...
                <p className="text-slate-300 text-sm mb-3">
                  Los pedidos se mueven automáticamente de "En Preparación" a "Entregados" 
                  cuando se marca como "Listo". Los pedidos entregados se pueden eliminar del historial.
                </p>
