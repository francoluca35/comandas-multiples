...
function Sidebar() {
  ...
  const handleLogoClick = () => {
    if (isExpanded) {
      // Si el sidebar está abierto, ir a la página de login de restaurantes y cerrar sesión
  ...
  return (
    <div
      className={`${
        isExpanded ? "w-64 md:w-72 lg:w-80" : "w-20"
      } bg-slate-900/80 backdrop-blur-sm border-r border-slate-700/50 flex flex-col items-center py-6 space-y-4 transition-all duration-300 ease-in-out shadow-2xl shadow-slate-900/50 h-screen max-h-screen overflow-hidden z-20 relative`}
    >
      {/* Foto de perfil del usuario */}
      <div
        className="cursor-pointer p-2 rounded-full hover:bg-slate-700/30 transition-all duration-300 mb-4"
      ...
      >
        <CloudinaryImage
          ...
          alt="Foto de perfil"
          size={50}
          className={isExpanded ? "w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24" : "w-14 h-14"}
