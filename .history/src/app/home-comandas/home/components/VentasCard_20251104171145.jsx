  ...
  const handleDescargarRegistroVentas = async () => {
    try {
      ...
      if (dias && dias.length > 0) {
        // Construir filas: Fecha | Salón | Takeaway | Delivery
        ...
        // Totales
        const totalSalon = filas.reduce((s, r) => s + (Number(r.Salón) || 0), 0);
        const totalTakeaway = filas.reduce((s, r) => s + (Number(r.Takeaway) || 0), 0);
        const totalDelivery = filas.reduce((s, r) => s + (Number(r.Delivery) || 0), 0);

        filas.push({ Fecha: "Total", Salón: totalSalon, Takeaway: totalTakeaway, Delivery: totalDelivery });
        ...
        const datosExcel = [
          { "Tipo de Venta": "Salón", Cantidad: contadoresAnterior.salon || contadores.salon },
          { "Tipo de Venta": "Takeaway", Cantidad: contadoresAnterior.takeaway || contadores.takeaway },
          { "Tipo de Venta": "Delivery", Cantidad: contadoresAnterior.delivery || contadores.delivery },
        ];
