...
function Sidebar() {
  ...
  useEffect(() => {
    ...
    } else if (pathname === "/home-comandas/gestion") {
    ...
    <div
      className={`${
        isExpanded
          ...
          : "w-16 sm:w-20"
      } fixed left-0 top-0 bg-slate-900/95 border-r border-slate-700/50 flex flex-col items-center py-3 sm:py-4 lg:py-6 space-y-2 sm:space-y-3 lg:space-y-4 transition-all duration-300 ease-in-out shadow-2xl shadow-slate-900/50 h-screen max-h-screen overflow-hidden z-20`}
    >
      ...
      <div className="flex flex-col items-center space-y-1 sm:space-y-2 md:space-y-3 w-full px-1 sm:px-2 md:px-3 flex-1 overflow-y-auto">
        ...
        {/* Gestión Centralizada - Solo ADMIN */}
        {permissions.canAccessGestion && (
          <div
            className={getItemClasses("gestion")}
            onClick={() => handleItemClick("gestion", "/home-comandas/gestion")}
          >
            <svg
              className={`w-5 h-5 ${
                activeItem === "gestion" ? "text-white" : ""
              }`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            ...
            {isExpanded && (
              <span
                className={`ml-3 font-semibold ${
                  activeItem === "gestion" ? "text-white" : "text-slate-300"
                }`}
              >
                Gestión
              </span>
