  ...
  const handleDescargarRegistroVentas = async () => {
    try {
      ...
      if (dias && dias.length > 0) {
        ...
        dias.forEach((d) => {
          filas.push({ "Tipo de Venta": "Salón", Fecha: d.fecha, Cantidad: d.salon || 0 });
          filas.push({ "Tipo de Venta": "Takeaway", Fecha: d.fecha, Cantidad: d.takeaway || 0 });
          filas.push({ "Tipo de Venta": "Delivery", Fecha: d.fecha, Cantidad: d.delivery || 0 });
        });

        // Calcular totales por tipo
        const totalSalon = filas.reduce((s, r) => s + (r["Tipo de Venta"] === "Salón" ? Number(r.Cantidad || 0) : 0), 0);
        const totalTakeaway = filas.reduce((s, r) => s + (r["Tipo de Venta"] === "Takeaway" ? Number(r.Cantidad || 0) : 0), 0);
        const totalDelivery = filas.reduce((s, r) => s + (r["Tipo de Venta"] === "Delivery" ? Number(r.Cantidad || 0) : 0), 0);

        // Agregar filas de totales debajo
        filas.push({ "Tipo de Venta": "Total Salón", Fecha: "", Cantidad: totalSalon });
        filas.push({ "Tipo de Venta": "Total Takeaway", Fecha: "", Cantidad: totalTakeaway });
        filas.push({ "Tipo de Venta": "Total Delivery", Fecha: "", Cantidad: totalDelivery });
        filas.push({ "Tipo de Venta": "Total General", Fecha: "", Cantidad: totalSalon + totalTakeaway + totalDelivery });
        ...
        alert("Registro de ventas descargado exitosamente. Los contadores han sido eliminados.");
      } else {
