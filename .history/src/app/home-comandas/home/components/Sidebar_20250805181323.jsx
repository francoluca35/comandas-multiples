...
function Sidebar() {
  ...
  useEffect(() => {
    // Obtener la imagen del usuario desde localStorage o datos del usuario
    const getUserImage = () => {
      if (usuario?.tipo === "restaurante") {
        // Para usuarios de restaurante, buscar en localStorage
        const userImageFromStorage = localStorage.getItem("userImage") || localStorage.getItem("imagen");
        if (userImageFromStorage) {
          setUserImage(userImageFromStorage);
        }
      } else if (usuario?.tipo === "superadmin") {
        // Para superadmin, usar datos del contexto
        // Aquí podrías obtener la imagen del usuario superadmin si la tienes
        setUserImage("");
      }
    };
    ...
    >
      ...
      >
        {userImage ? (
          <img
            src={userImage}
            alt="Foto de perfil"
            className="w-10 h-10 rounded-full object-cover border-2 border-slate-600 hover:border-slate-400 transition-all duration-300"
            onError={(e) => {
        ...
        {(!userImage || userImage === "") && (
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-sm border-2 border-slate-600 hover:border-slate-400 transition-all duration-300">
            {usuario?.usuario?.charAt(0)?.toUpperCase() || usuario?.email?.charAt(0)?.toUpperCase() || "U"}
          </div>
        )}
      </div>
      ...
      {isExpanded && (
        <div className="flex items-center space-x-3 mb-8 px-4">
          ...
          <span className="text-white font-bold text-xl bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent">
            Comandas
          </span>
        </div>
      )}
