...
function VentasCard() {
  ...
  const handleDescargarRegistroVentas = async () => {
    try {
      // Obtener mes actual (ya que recién empezamos con contadores diarios)
      const ahora = new Date();
      const mesStr = `${ahora.getFullYear()}-${String(
        ahora.getMonth() + 1
      ).padStart(2, "0")}`;

      // Obtener los contadores diarios del mes actual
      let dias = [];
      try {
        dias = await obtenerContadoresDiariosMes(mesStr);
      } catch (err) {
      ...
      if (dias && dias.length > 0) {
        ...
        const nombreArchivo = `Registro_Ventas_${mesStr}.xlsx`;
        XLSX.writeFile(workbook, nombreArchivo);

        // Eliminar contadores (mensual + diarios) luego de exportar
        try {
          await eliminarContadoresMes(mesStr);
        } catch (err) {
        ...
        const contadoresMes = await obtenerContadoresMes(mesStr);

        const datosExcel = [
          {
            "Tipo de Venta": "Salón",
            Cantidad: contadoresMes.salon || contadores.salon,
          },
          {
            "Tipo de Venta": "Takeaway",
            Cantidad: contadoresMes.takeaway || contadores.takeaway,
          },
          {
            "Tipo de Venta": "Delivery",
            Cantidad: contadoresMes.delivery || contadores.delivery,
          },
        ];
