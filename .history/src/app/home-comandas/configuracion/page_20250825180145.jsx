...
function ConfiguracionContent() {
  ...
  useEffect(() => {
    if (!restaurantLoading && !mpLoading) {
      checkFirstTimeSetup();
    }
  }, [user, restaurant, restaurantLoading, mpLoading]);
  ...
  const handleMercadoPagoConfig = async () => {
    ...
    try {
      ...
      if (
        ...
        alert(
          "Por favor completa todos los campos de Mercado Pago (Access Token, Public Key y Webhook Secret)"
        );
        return;
      }
      ...
      try {
        ...
        console.log("✅ Credenciales validadas:", validationResult.accountInfo);
      } catch (validationError) {
        console.error("Error validando credenciales:", validationError);
        alert(`Error al validar las credenciales: ${validationError.message}`);
        return;
      }
  ...
  if (loading || restaurantLoading || mpLoading) {
    return (
      ...
      >
        ...
        <div className="flex-1 overflow-auto bg-gradient-to-br from-slate-800 via-slate-900 to-slate-800">
          <div className="max-w-4xl mx-auto px-6 py-6">
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-blue-200 text-sm">
                    Aquí puedes gestionar la configuración de Mercado Pago, la
                    información de tu restaurante y tus huellas digitales.
                  </p>
                </div>
              </div>
            </div>
                  ...
                  {mercadopagoConfig.isConfigured ? (
                        ...
                        <div>
                          ...
                          {mpConfig && (
                            <div className="mt-2 text-xs text-green-200">
                              <p>
                                Tipo de cuenta:{" "}
                                {mpConfig.accountType === "sandbox"
                                  ? "Sandbox (Pruebas)"
                                  : "Producción"}
                              </p>
                              {mpConfig.lastValidated && (
                                <p>
                                  Última validación:{" "}
                                  {new Date(
                                    mpConfig.lastValidated.toDate()
                                  ).toLocaleDateString()}
                                </p>
                              )}
                            </div>
                    ...
                    <>
                      ...
                      <div>
                        ...
                        />
                        <p className="text-xs text-slate-400 mt-1">
                          Clave secreta para verificar webhooks. Encuéntrala en
                          tu panel de desarrolladores de MercadoPago → Webhooks
                        </p>
                      </div>
            ...
            <div className="mb-6">
              <BiometricSettings 
                userId={user?.id || localStorage.getItem("usuarioId")} 
                username={user?.usuario || localStorage.getItem("usuario")} 
              />
            </div>
