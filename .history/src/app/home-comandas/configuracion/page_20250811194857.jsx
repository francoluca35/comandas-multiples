...
function ConfiguracionContent() {
  ...
  const handleMercadoPagoConfig = async () => {
    ...
    try {
      ...
      if (!mercadopagoConfig.accessToken.startsWith('APP_USR-') && !mercadopagoConfig.accessToken.startsWith('TEST-')) {
        alert("El Access Token debe comenzar con 'APP_USR-' o 'TEST-'");
        return;
      }

      if (!mercadopagoConfig.publicKey.startsWith('APP_USR-') && !mercadopagoConfig.publicKey.startsWith('TEST-')) {
      ...
      try {
        const validationResponse = await fetch('/api/pagos/validate-credentials', {
          method: 'POST',
          headers: {
        ...
        if (!validationData.isValid) {
          alert('Las credenciales de Mercado Pago no son válidas. Por favor verifica e intenta nuevamente.');
          return;
        }
      } catch (validationError) {
        console.error('Error validando credenciales:', validationError);
        alert('Error al validar las credenciales. Por favor intenta nuevamente.');
        return;
      }
      ...
      await updateDoc(doc(db, "restaurantes", restaurant.id), {
        mercadopago: {
          ...
          accountType: mercadopagoConfig.accessToken.startsWith('TEST-') ? 'sandbox' : 'production'
        },
