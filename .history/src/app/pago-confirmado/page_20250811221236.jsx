...
export default function PagoConfirmadoPage() {
  ...
  // Hooks
  const { paymentStatus, isApproved, isPending, isRejected, loading } = usePaymentStatus(externalReference);
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
        <div className="text-center">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">${paymentStatus.amount?.toLocaleString() || "0"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Método:</span>
                  <span className="font-semibold">{paymentStatus.paymentMethod || "N/A"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Referencia:</span>
                  <span className="font-semibold text-xs">{paymentStatus.externalReference || "N/A"}</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            {isApproved && (
              <button
                onClick={() => paymentStatus?.orderData && printTicket(paymentStatus.orderData)}
          ...
          {isApproved && (
            <div className="mt-6 p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-sm text-green-800">
                El pago ha sido procesado exitosamente y la mesa ha sido liberada automáticamente.
              </p>
            </div>
          )}
        </div>
