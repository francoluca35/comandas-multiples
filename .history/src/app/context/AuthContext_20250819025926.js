...
export const AuthProvider = ({ children }) => {
  ...
  const checkAuth = useCallback(async () => {
    ...
    if (isSuperAdminSystem) {
        ...
        if (savedEmail && savedRol === "superadmin") {
          ...
          try {
            ...
            if (snap.exists()) {
              ...
              console.log(
                "❌ Usuario guardado ya no existe en la base de datos"
              );
              setUsuario(null);
              setRol(null);
              localStorage.removeItem("superadmin_email");
      ...
      // Para restaurantes, solo necesitamos verificar que exista la información básica del restaurante
      // No necesitamos usuario logueado para mostrar la página de login
      if (nombreResto && codActivacion) {
        // Si hay información del restaurante, permitir acceso
        setUsuario({
          tipo: "restaurante",
          nombreResto,
          codActivacion: !!codActivacion,
        ...
        setRol("restaurante");
        console.log("✅ Información de restaurante encontrada:", { nombreResto });
      } else if (restauranteId && usuarioLocal) {
        // Si hay usuario logueado, mantener la sesión
        setUsuario({
  ...
  useEffect(() => {
    ...
    // Solo escuchar cambios en Firebase Auth si estamos en el sistema de superadmin
    const currentPath = window.location.pathname;
    const isSuperAdminSystem = currentPath.includes("/home-master");

    if (isSuperAdminSystem) {
      console.log(
        "🔍 Configurando listener de Firebase Auth para superadmin..."
      );
      ...
      console.log(
        "🔍 No configurando listener de Firebase Auth (sistema de restaurantes)"
      );
    }
  }, []);
