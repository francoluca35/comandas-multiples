    ...
    const cargarRestaurante = async () => {
      try {
        ...
        if (restauranteSnap.exists()) {
          ...
          if (data.codigoActivacion === codActivacion || data.codActivacion === codActivacion) {
            ...
            console.warn(
              "⚠️ Código de activación no coincide para:",
              restauranteId
            );
          ...
          console.log(
            "🔍 Restaurante no encontrado con ID generado, buscando por nombre..."
          );
          ...
          try {
            ...
            restaurantesSnap.docs.forEach((doc) => {
              ...
              if (
                data.nombre &&
                data.nombre.toLowerCase() === nombreResto.toLowerCase()
              ) {
            ...
            if (restauranteEncontrado) {
              ...
              console.log(
                "✅ Restaurante encontrado por nombre:",
                restauranteEncontrado.id
              );
            } else {
              console.warn(
                "⚠️ Restaurante no encontrado con nombre:",
                nombreResto
              );
              // Limpiar datos inválidos
              limpiarRestaurante();
            ...
            console.warn(
              "⚠️ Error al buscar restaurante por nombre:",
              searchError
            );
            // Limpiar datos inválidos
            limpiarRestaurante();
