...
export default function VistaRestaurantes() {
  ...
  const getFilteredRestaurants = () => {
    ...
    switch (selectedFilter) {
      ...
      case "warnings":
        return filtered.filter(restaurant => restaurant.warnings > 0 && restaurant.criticalIssues === 0);
      case "healthy":
        return filtered.filter(restaurant => restaurant.issues.length === 0);
      case "active":
        return filtered.filter(restaurant => restaurant.estado === "activo" || restaurant.estado === "habilitado");
      default:
        return filtered;
    }
  };
    ...
    <div className="text-white p-6 space-y-6">
        ...
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">{monitoringData.totalRestaurants}</p>
            </div>
        ...
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-400">{monitoringData.activeRestaurants}</p>
            </div>
        ...
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-red-400">{monitoringData.restaurantsWithIssues}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm font-medium">
                {monitoringData.lastScan ? formatDate(monitoringData.lastScan) : "N/A"}
              </p>
              <p className="text-xs text-gray-500">
                {monitoringData.scanDuration > 0 ? `${monitoringData.scanDuration}ms` : ""}
              </p>
            </div>
          ...
          {Object.entries(issuesByCategory).map(([category, counts]) => (
            <div key={category} className="bg-gray-700 rounded-lg p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                {category === "tables" && <FaTable className="text-green-500" />}
                {category === "products" && <FaBox className="text-yellow-500" />}
                {category === "payments" && <FaCreditCard className="text-purple-500" />}
                {category === "orders" && <FaChartBar className="text-orange-500" />}
                {category === "structure" && <FaBug className="text-red-500" />}
                {category === "performance" && <FaClock className="text-indigo-500" />}
                {category === "system" && <FaExclamationTriangle className="text-gray-500" />}
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex space-x-2">
          ...
          >
            Advertencias ({restaurants.filter(r => r.warnings > 0 && r.criticalIssues === 0).length})
          </button>
          <button
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-700">
              {filteredRestaurants.map((restaurant) => (
                <tr key={restaurant.id} className="hover:bg-gray-700 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(restaurant.estado)}`}>
                      {restaurant.estado}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold">Detalles: {selectedRestaurant.nombre}</h3>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div><span className="text-gray-400">ID:</span> {selectedRestaurant.id}</div>
                  <div><span className="text-gray-400">Estado:</span> {selectedRestaurant.estado}</div>
                  <div><span className="text-gray-400">Fecha Creación:</span> {formatDate(selectedRestaurant.fechaCreacion)}</div>
                  <div><span className="text-gray-400">Última Actividad:</span> {formatDate(selectedRestaurant.ultimaActividad)}</div>
                </div>
              </div>
              <div>
                <h4 className="font-semibold mb-2">Resumen de Problemas</h4>
                <div className="space-y-2 text-sm">
                  <div><span className="text-gray-400">Total Problemas:</span> {selectedRestaurant.issues.length}</div>
                  <div><span className="text-red-400">Críticos:</span> {selectedRestaurant.criticalIssues}</div>
                  <div><span className="text-yellow-400">Advertencias:</span> {selectedRestaurant.warnings}</div>
                </div>
              </div>
            </div>
            ...
            {selectedRestaurant.issues.length > 0 ? (
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <span className={`font-medium ${
                              issue.type === "critical" ? "text-red-400" : "text-yellow-400"
                            }`}>
                              {issue.message}
                            </span>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-green-400 font-medium">Sin problemas detectados</p>
                <p className="text-gray-400 text-sm">Este restaurante está funcionando correctamente</p>
              </div>
            )}
          </div>
        </div>
      ...
      {error && (
        <div className="bg-red-900/20 border border-red-700 rounded-lg p-4">
          <div className="flex items-center space-x-2">
            <FaExclamationTriangle className="text-red-500" />
            <span className="text-red-400 font-medium">Error en el escaneo:</span>
          </div>
