...
export default function VistaActivacion() {
  ...
  const precioCalculado = useMemo(() => {
    if (watchedValues.tipoServicio && watchedValues.periodicidad) {
      return precios[watchedValues.tipoServicio]?.[watchedValues.periodicidad] || 0;
    }
    return 0;
  ...
  const generatePDF = useCallback((data) => {
    ...
    doc.setFontSize(12);
    doc.setTextColor(200, 200, 200);
    doc.text(`Tipo de Servicio: ${data.tipoServicio === 'conFinanzas' ? 'Con Finanzas' : 'Sin Finanzas'}`, 20, 140);
    doc.text(`Forma de Pago: ${data.formaPago}`, 20, 150);
    doc.text(`Periodicidad: ${data.periodicidad === 'mensual' ? 'Mensual' : 'Anual'}`, 20, 160);
    ...
    // Guardar PDF
    doc.save(`contrato_${data.nombre.replace(/\s+/g, '_')}_${new Date().getTime()}.pdf`);
  }, [precioCalculado]);
  ...
  const onSubmit = useCallback(
    async (data) => {
      try {
        ...
        const restaurantData = {
          ...
          proximoPago: data.periodicidad === 'mensual' 
    ...
    [createRestaurantMutation, reset, closeModal, clearError, handleError, generatePDF, precioCalculado]
  );
