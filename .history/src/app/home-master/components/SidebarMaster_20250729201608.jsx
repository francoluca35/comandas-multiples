"use client";
import { useState } from "react";
import {
  FaBars,
  FaHome,
  FaShoppingCart,
  FaDollarSign,
  FaBoxes,
  FaConciergeBell,
  FaChartBar,
  FaTags,
  FaUserCog,
  FaSlidersH,
  FaPowerOff,
} from "react-icons/fa";

const opciones = [
  { id: "principal", label: "Principal", icon: FaHome },
  { id: "ventas", label: "Ventas", icon: FaShoppingCart },
  { id: "dinero", label: "Dinero", icon: FaDollarSign },
  { id: "stock", label: "Stock", icon: FaBoxes },
  { id: "cocina", label: "Cocina", icon: FaConciergeBell },
  { id: "analisis", label: "Análisis", icon: FaChartBar },
  { id: "gestion", label: "Gestión", icon: FaTags },
  { id: "perfil", label: "Perfil", icon: FaUserCog },
  { id: "ajustes", label: "Ajustes", icon: FaSlidersH },
];

export default function Sidebar({ onChangeVista }) {
  const [abierto, setAbierto] = useState(true);
  const [opcionActiva, setOpcionActiva] = useState("ventas");

  return (
    <div
      className={`h-screen bg-[#111] transition-all duration-300 ${
        abierto ? "w-48" : "w-14"
      } flex flex-col justify-between`}
    >
      {/* Sección superior */}
      <div>
        {/* Botón de toggle */}
        <div
          className={`flex items-center ${
            abierto ? "justify-start pl-3" : "justify-center"
          } h-14 text-white font-bold`}
        >
          <button
            onClick={() => setAbierto(!abierto)}
            className="focus:outline-none"
          >
            <FaBars size={20} />
          </button>
          {abierto && <span className="ml-3 text-sm">admin</span>}
        </div>

        {/* Opciones */}
        <div className="flex flex-col gap-1 mt-2">
          {opciones.map(({ id, label, icon: Icon }) => {
            const activo = opcionActiva === id;

            return (
              <button
                key={id}
                onClick={() => {
                  setOpcionActiva(id);
                  onChangeVista?.(id);
                }}
                className={`flex items-center gap-3 h-10 px-3 rounded-l-full transition-all w-full
                  ${
                    activo
                      ? "bg-white text-black font-bold"
                      : "text-white hover:bg-white/20"
                  }
                  ${abierto ? "justify-start" : "justify-center"}
                `}
              >
                <Icon size={18} />
                {abierto && <span className="text-sm">{label}</span>}
              </button>
            );
          })}
        </div>
      </div>

      {/* Botón de salida */}
      <div className="mb-4">
        <button
          onClick={() => alert("Cerrar sesión")}
          className={`flex items-center gap-3 h-10 px-3 rounded-l-full text-white hover:bg-red-600 w-full
            ${abierto ? "justify-start" : "justify-center"}
          `}
        >
          <FaPowerOff size={18} />
          {abierto && <span className="text-sm">Salir</span>}
        </button>
      </div>
    </div>
  );
}
