...
function PreLogin() {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        if (data.email === email && data.password === password) {
          ...
          // Guardar datos del restaurante para la pantalla de bienvenida
          setRestauranteData({
            nombre: data.resto,
            cantUsuarios: data.cantUsuarios,
            logo: data.logo || ""
          });

          // Mostrar pantalla de bienvenida en lugar de alert
          setShowWelcome(true);
  ...
  const PantallaBienvenida = () => {
    ...
    useEffect(() => {
      // Mostrar el cartel después de un pequeño delay
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 text-center">
          {/* Logo QS */}
          <div className="mb-8">
            <div className="w-40 h-40 bg-gradient-to-br from-purple-500 to-blue-600 rounded-full flex items-center justify-center mx-auto shadow-2xl animate-pulse">
              <div className="text-center">
                <div className="text-white font-bold text-5xl mb-1">QS</div>
                <div className="text-white text-sm font-medium">Quick Solution</div>
              </div>
            </div>
          </div>

          {/* Título Quick Solution */}
          <h1 
            className="text-6xl font-bold text-white mb-8 animate-fade-in"
          ...
          {showCartel && (
            <div className="animate-slide-up">
              <div className="bg-black bg-opacity-80 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border border-white border-opacity-30 max-w-md mx-auto">
                {/* Logo CM */}
                <div className="w-20 h-20 bg-gradient-to-br from-purple-500 to-blue-600 rounded-full flex items-center justify-center mx-auto mb-6">
                  <div className="text-center">
                    <div className="text-white font-bold text-2xl">CM</div>
                    <div className="text-white text-xs">Comandas</div>
                    <div className="text-white text-xs">Multiples</div>
                  </div>
                </div>
                ...
                <h3 className="text-xl font-bold text-white mb-4">
                  {restauranteData?.nombre?.toUpperCase()}
                </h3>
                <p className="text-white text-lg mb-6">
                  Tienes {restauranteData?.cantUsuarios} Usuarios permitidos!
                </p>

                {/* Botón continuar */}
                <button
                  onClick={handleContinuar}
                  className="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 px-8 rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg"
                >
                  Continuar
                </button>
              </div>
  ...
  // Si showWelcome es true, mostrar la pantalla de bienvenida
  if (showWelcome) {
