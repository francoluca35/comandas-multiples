  ...
  const registrarInicioSesion = async () => {
    return preventMultipleCalls(async () => {
      try {
        ...
        // Buscar sesión abierta del usuario (horaCierre === null)
        ...
        if (!snapshot.empty) {
          console.log(
            "Ya existe una sesión abierta para este usuario. No se crea un nuevo documento."
          );
        ...
        await addDoc(historialRef, {
          ...
          horaApertura: ahora,
          horaCierre: null, // queda null hasta que se cierre la sesión
          timestamp: ahora,
        });
  ...
  const registrarCierreSesion = async () => {
    return preventMultipleCalls(async () => {
      try {
        ...
        // Traer todas las sesiones abiertas del usuario
        const snapshot = await getDocs(
          query(
            historialRef,
            where("usuarioId", "==", usuarioId),
        ...
        // Tomar la más reciente según horaApertura
        let ultimaSesionDoc = null;
        snapshot.forEach((docu) => {
          const data = docu.data();
          if (
            !ultimaSesionDoc ||
            new Date(data.horaApertura) >
              new Date(ultimaSesionDoc.data().horaApertura)
          ) {
            ultimaSesionDoc = docu;
          }
        });

        if (!ultimaSesionDoc) {
          console.log("No se encontró sesión abierta para cerrar");
          return;
        }

        // Actualizar el documento existente con horaCierre
        await updateDoc(ultimaSesionDoc.ref, { horaCierre: ahora });
