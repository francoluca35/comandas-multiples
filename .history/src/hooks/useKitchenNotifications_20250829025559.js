...
const useKitchenNotifications = () => {
  ...
  // Función helper para crear objeto de configuración
  const createConfigObject = (updates = {}) => {
    ...
    // Solo agregar campos de archivo personalizado si existen
    if (updates.customAudioData !== undefined) {
      config.customAudioData = updates.customAudioData;
    } else if (customAudioData) {
      config.customAudioData = customAudioData;
    }

    if (updates.customAudioName !== undefined) {
      config.customAudioName = updates.customAudioName;
    } else if (customAudioFile?.name) {
      config.customAudioName = customAudioFile.name;
    }

    if (updates.customAudioMimeType !== undefined) {
      config.customAudioMimeType = updates.customAudioMimeType;
    } else if (customAudioFile?.type) {
      config.customAudioMimeType = customAudioFile.type;
    }
  ...
  const saveConfiguration = async (config = {}) => {
    ...
    try {
      const configToSave = createConfigObject(config);
