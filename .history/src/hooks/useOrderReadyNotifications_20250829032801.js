      ...
      snapshot.docChanges().forEach((change) => {
        if (change.type === 'added') {
          ...
          if (!notificationData.leida) {
            ...
            // Marcar como leída después de un pequeño delay usando doc() para crear la referencia
            setTimeout(async () => {
              try {
                const docRef = doc(db, `restaurantes/${restaurant.id}/notificaciones`, change.doc.id);
                await updateDoc(docRef, { leida: true });
                console.log("✅ Notificación marcada como leída:", change.doc.id);
              } catch (error) {
