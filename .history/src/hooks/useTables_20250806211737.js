...
export const useTables = () => {
  ...
  const deleteTable = async (tableId) => {
    ...
    try {
      ...
      // Reordenar todas las mesas después de eliminar
      await reorderAllTablesAfterDelete();

      // Recargar las mesas para mostrar el nuevo orden
      await fetchTables();
    } catch (err) {
  ...
  const reorderAllTablesAfterDelete = async () => {
    // Reordenar mesas de "adentro" por separado
    const adentroTables = getTablesByLocation("adentro").sort(
      (a, b) => parseInt(a.numero) - parseInt(b.numero)
    );

    // Reordenar mesas de "afuera" por separado
    const afueraTables = getTablesByLocation("afuera").sort(
      (a, b) => parseInt(a.numero) - parseInt(b.numero)
    );

    // Reordenar mesas de "adentro" secuencialmente (1, 2, 3, ...)
    for (let i = 0; i < adentroTables.length; i++) {
      const table = adentroTables[i];
    ...
    // Reordenar mesas de "afuera" secuencialmente (1, 2, 3, ...)
    for (let i = 0; i < afueraTables.length; i++) {
      const table = afueraTables[i];
      const newNumber = String(i + 1).padStart(2, "0");
