...
export const useRestaurantMonitoring = () => {
  ...
  const analyzeAndResolveIssue = useCallback(async (restaurantId, issue) => {
    try {
      console.log(`🔧 Analizando problema: ${issue.category} - ${issue.message}`);
    ...
    try {
      ...
      if (usuariosSnap.empty) {
        ...
        return {
          ...
          solution: "Usuario: admin, Contraseña: admin123 (cambiar inmediatamente)"
        };
      }
      ...
      if (inactiveUsers.length > 0) {
        return {
          ...
          solution: "Contactar a los usuarios para reactivar sus cuentas o eliminarlas"
        };
      }
  ...
  const resolveTablesIssue = async (restaurantId, issue) => {
    try {
      ...
      const mesas = mesasSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      const inconsistentTables = mesas.filter(mesa => 
        mesa.estado === "desconocido" ||
      ...
      if (inconsistentTables.length > 0) {
        ...
        for (const mesa of inconsistentTables) {
          const mesaRef = doc(db, "restaurantes", restaurantId, "mesas", mesa.id);
          let newEstado = "libre";
          
          if (mesa.estado === "ocupado" && (!mesa.productos || mesa.productos.length === 0)) {
            newEstado = "libre";
          } else if (mesa.estado === "libre" && mesa.productos && mesa.productos.length > 0) {
            newEstado = "ocupado";
    ...
    try {
      ...
      if (menuSnap.empty) {
        ...
        for (const categoria of defaultCategories) {
          ...
          const productosRef = collection(db, "restaurantes", restaurantId, "menus", categoriaRef.id, "productos");
  ...
  const resolveOrdersIssue = async (restaurantId, issue) => {
    try {
      ...
      const pedidosSnap = await getDocs(pedidosQuery);
      const incompleteOrders = pedidosSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const resolvePerformanceIssue = async (restaurantId, issue) => {
    return {
      ...
      solution: "Optimizar consultas de base de datos y verificar conexión a internet"
    };
  };
