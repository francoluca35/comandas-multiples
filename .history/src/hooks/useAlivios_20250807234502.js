...
export const useAlivios = () => {
  ...
  const fetchAlivios = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/alivios?restauranteId=${restauranteId}`);
  ...
  const crearAlivio = async (nombreServicio, monto, tipodepago) => {
    setLoading(true);
    setError(null);
    try {
      const restauranteId = getRestaurantId();
  ...
  const actualizarAlivio = async (servicioId, nombreServicio, monto, tipodepago) => {
    setLoading(true);
    setError(null);
    try {
      const restauranteId = getRestaurantId();
  ...
  const eliminarAlivio = async (servicioId) => {
    ...
    try {
      const restauranteId = getRestaurantId();
      const response = await fetch(`/api/alivios?restauranteId=${restauranteId}&servicioId=${servicioId}`, {
        method: "DELETE",
      });
