...
export const useAlivios = () => {
  ...
  const getTotalAlivios = () => {
    ...
    if (Array.isArray(alivios.alivios)) {
      const total = alivios.alivios.reduce((sum, alivio) => sum + (alivio.monto || 0), 0);
      console.log("💰 Total alivios calculado desde array:", { alivios: alivios.alivios, total });
      return total;
    }
    // Si no, usar el totalAlivios del estado
    const total = alivios.totalAlivios || 0;
