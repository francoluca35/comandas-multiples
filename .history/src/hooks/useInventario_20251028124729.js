...
export const useInventario = () => {
  ...
  const createBebida = async (bebidaData) => {
    ...
    try {
      ...
      // NO sincronizar automáticamente con menú de bebidas
      // Las bebidas del inventario aparecerán automáticamente en la vista de productos
      // a través del hook useProducts.js
  ...
  const updateMateriaPrima = async (materiaPrimaId, updateData) => {
    ...
    try {
      ...
      // Si es comida, actualizar también en menú
      if (updateData.esComida) {
        console.log("🍽️ Actualizando comida en menú:", updateData.nombre);

        // Buscar el producto en el menú para actualizarlo
        // Por ahora solo actualizamos en materia prima
        // TODO: Implementar búsqueda y actualización en menú
      }
  ...
  const getProductosStockBajo = () => {
    ...
    const materiaPrimaStockBajo = materiaPrima.filter((mp) => {
  ...
  const getProductosSinStock = () => {
    ...
    const materiaPrimaSinStock = materiaPrima.filter((mp) => {
      const stockValue = Number(mp.stock) || 0;
      return stockValue === 0;
    });
  ...
  const getInventarioStats = () => {
    ...
    const valorEnStockBebidas = bebidas.reduce((total, bebida) => {
      const stockValue = Number(bebida.stock) || 0;
      const precioValue = Number(bebida.precio) || 0;
      return total + (stockValue * precioValue);
    }, 0);
    const valorEnStockMateriaPrima = materiaPrima.reduce((total, mp) => {
      const stockValue = Number(mp.stock) || 0;
      const precioValue = Number(mp.precio) || 0;
      return total + (stockValue * precioValue);
    }, 0);
    const valorEnStock = valorEnStockBebidas + valorEnStockMateriaPrima;

    // Costo de Stock = cantidad × costo
    const costoStockBebidas = bebidas.reduce((total, bebida) => {
      const stockValue = Number(bebida.stock) || 0;
      const costoValue = Number(bebida.costo) || 0;
      return total + (stockValue * costoValue);
    }, 0);
    const costoStockMateriaPrima = materiaPrima.reduce((total, mp) => {
      const stockValue = Number(mp.stock) || 0;
      const costoValue = Number(mp.costo) || 0;
      return total + (stockValue * costoValue);
    }, 0);
  ...
  const getStockStats = () => {
    ...
    const totalStock = [...bebidas, ...materiaPrima].reduce((total, producto) => {
      const stockValue = Number(producto.stock) || 0;
      return total + stockValue;
    }, 0);

    const valorEnStock = [...bebidas, ...materiaPrima].reduce((total, producto) => {
      const stockValue = Number(producto.stock) || 0;
      const precioValue = Number(producto.precio) || 0;
      return total + (stockValue * precioValue);
    }, 0);

    const costoStock = [...bebidas, ...materiaPrima].reduce((total, producto) => {
      const stockValue = Number(producto.stock) || 0;
