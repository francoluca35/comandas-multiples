...
const useOrderReadyNotifications = () => {
  ...
  // Función para mostrar notificación del navegador de forma segura
  const showBrowserNotification = useCallback((title, message) => {
    // Verificar si las notificaciones están disponibles
    if (!('Notification' in window)) {
      console.log("🔔 Notificaciones del navegador no soportadas");
      return;
    }
    ...
    if (typeof Notification !== 'function') {
      console.log("🔔 Constructor Notification no disponible");
      return;
    }
  ...
  const showOrderReadyNotification = useCallback((notificationData) => {
    ...
    showBrowserNotification('Pedido Listo', message);
  ...
  }, [isEnabled, playSonido1, vibrateDevice, showBrowserNotification]);
