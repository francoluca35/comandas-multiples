...
export const useTicketSender = () => {
  ...
  const sendTicketWhatsApp = async (orderData, pdfBlob, whatsappNumber) => {
    ...
    try {
      ...
      // Intentar usar Web Share API con archivos (envío directo)
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        try {
          ...
          console.log("✅ Ticket enviado directamente por WhatsApp");
          return true;
        } catch (shareError) {
          console.log("⚠️ Error con Web Share, usando método alternativo:", shareError);
          // Continuar con el método alternativo
        }
      }

      // Método alternativo: descargar PDF y abrir WhatsApp
      // 1. Descargar automáticamente el PDF
      ...
      // 2. Esperar un momento para que se complete la descarga
      await new Promise(resolve => setTimeout(resolve, 1000));

      // 3. Abrir WhatsApp Web directamente con el número del cliente
      ...
      alert(
        "📱 WhatsApp abierto + PDF descargado\n\n" +
        "1. ✅ El PDF se descargó automáticamente\n" +
        "2. ✅ WhatsApp Web se abrió con el chat del cliente\n" +
        "3. 📎 Arrastra el PDF descargado al chat de WhatsApp\n" +
        "4. 📤 Envía el mensaje\n\n" +
        `Cliente: ${orderData.cliente || 'Cliente'}\n` +
