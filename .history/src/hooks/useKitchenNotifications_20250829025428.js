...
const useKitchenNotifications = () => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        try {
          ...
          console.error("Error guardando archivo personalizado:", error);
        }
      };
  ...
  return {
    ...
    setIsEnabled: useCallback(async (enabled) => {
      ...
      await saveConfiguration({ isEnabled: enabled });
    }, [saveConfiguration]),
    
    setVolume: useCallback(async (newVolume) => {
      setVolume(newVolume);
      await saveConfiguration({ volume: newVolume });
    }, [saveConfiguration]),
    
    setSoundType: useCallback(async (newSoundType) => {
      setSoundType(newSoundType);
      await saveConfiguration({ soundType: newSoundType });
    }, [saveConfiguration]),
