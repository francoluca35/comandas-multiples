...
export const useHistorialEmpleados = () => {
  ...
  const registrarInicioSesion = async () => {
    if (isProcessing.current) {
      console.log("⏳ Ya hay un proceso en ejecución, esperando...");
      return;
    }
    ...
    try {
      ...
      if (!usuarioId || !restauranteId || !usuario) {
        const errorMsg = `Falta información: usuarioId=${usuarioId}, restauranteId=${restauranteId}, usuario=${usuario}`;
      ...
      try {
        ...
        const qOpenSimple = query(
          ...
          where("usuarioId", "==", usuarioId),
          where("horaCierre", "==", null),
          limit(10) // Obtener más para filtrar manualmente
        );
        snapOpen = await getDocs(qOpenSimple);
