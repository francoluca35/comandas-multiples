...
export const useSueldos = () => {
  ...
  const fetchSueldos = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/sueldos?restauranteId=${restauranteId}`);
  ...
  const getTotalSueldos = useCallback(() => {
    try {
      ...
      const total = sueldos.reduce((sum, empleado) => {
        if (empleado.historialPagos && typeof empleado.historialPagos === 'object') {
          const empleadoTotal = Object.values(empleado.historialPagos).reduce((empleadoSum, pago) => {
  ...
  const getCantidadSueldos = useCallback(() => {
    try {
      if (!Array.isArray(sueldos) || sueldos.length === 0) {
        return 0;
      }
