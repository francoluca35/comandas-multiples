...
export const useTables = () => {
  ...
  const setupRealtimeListener = () => {
    try {
      const restaurantId = getRestaurantId();
      console.log(
        "🔔 Configurando listener en tiempo real para mesas:",
        restaurantId
      );
  ...
  const markTableAsPaid = async (
    ...
    try {
      ...
      // Obtener los datos actuales de la mesa para preservar el total
      const tableDoc = await getDoc(tableRef);
      ...
      if (!tipoFinal) {
        ...
        if (
          numeroMesa.toString().toUpperCase() === "TAKEAWAY" ||
          numeroMesa.toString().includes("TAKEAWAY")
        ) {
          tipoFinal = "takeaway";
        } else if (
          numeroMesa.toString().toUpperCase() === "DELIVERY" ||
          numeroMesa.toString().includes("DELIVERY")
        ) {
          tipoFinal = "delivery";
        } else {
          tipoFinal = "salon";
      ...
      // Incrementar contador de ventas (mensual) y contador diario
      try {
        ...
        try {
          ...
          if (dailySnap.exists()) {
            ...
            const nuevo = (ddata[tipoFinal] || 0) + 1;
          ...
          console.error(
            "Error actualizando contador diario (mesas):",
            dailyErr
          );
        }
      } catch (err) {
