...
// Función para guardar credenciales en IndexedDB
...
// Función para eliminar credencial de IndexedDB
const deleteCredentialFromDB = async (credentialId) => {
  const db = await initDB();
  const transaction = db.transaction(["credentials"], "readwrite");
  const store = transaction.objectStore("credentials");
...
export const useBiometricAuth = () => {
  ...
  const registerBiometric = useCallback(async (userId, username) => {
    ...
    try {
      ...
      // Guardar en IndexedDB para persistencia local
