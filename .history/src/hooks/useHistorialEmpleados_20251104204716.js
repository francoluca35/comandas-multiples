...
export const useHistorialEmpleados = () => {
  ...
  const registrarInicioSesion = async () => {
    try {
      ...
      const rol = localStorage.getItem("rol") || localStorage.getItem("role") || "Empleado";
  ...
  const registrarCierreSesion = async () => {
    try {
      ...
      for (const documento of snapshot.docs) {
        ...
        if (!datos.horaCierre) {
          const docRef = doc(db, "restaurantes", restauranteId, "historialEmpleados", documento.id);
          await updateDoc(docRef, {
            horaCierre: new Date(),
            cierreTimestamp: new Date(),
          });
          encontrado = true;
          console.log(`✅ Actualizado documento ${documento.id} con horaCierre`);
          break;
        }
      }
      ...
      if (!encontrado) {
        ...
        console.log("⚠️ No se encontró apertura previa: creado documento de cierre");
      }
