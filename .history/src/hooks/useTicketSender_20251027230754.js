  ...
  const sendTicketWhatsApp = async (orderData, pdfBlob, whatsappNumber) => {
    ...
    try {
      ...
      if (navigator.share && navigator.canShare && navigator.canShare({ files: [file] })) {
        ...
        // Fallback: abrir WhatsApp Web directamente con el número del cliente
        const message = encodeURIComponent(
          ...
          `¡Hola! Aquí tienes tu ticket de compra.`
        );
        
        // Abrir directamente el chat con el número específico
        window.open(`https://wa.me/${formattedNumber}?text=${message}`, '_blank');
        
        console.log(`✅ WhatsApp abierto para ${formattedNumber}`);
