...
export const useTables = () => {
  ...
  const moveMultipleTablesToLocation = async (startNumber, endNumber, fromLocation, toLocation) => {
    ...
    for (let i = 0; i < remainingTables.length; i++) {
      const table = remainingTables[i];
      const newTableNumber = String(parseInt(table.numero) + tablesToMove.length).padStart(2, "0");
      await updateTable(table.id, { numero: newTableNumber });
    }
  };
