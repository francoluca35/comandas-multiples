...
import { useAuth, useRestaurant, useDashboard } from "../store";
  ...
  return useMutation({
    ...
    onMutate: async (newRestaurant) => {
      ...
      queryClient.setQueryData(["restaurants"], (old) => {
        return old ? [...old, { ...newRestaurant, id: "temp-id" }] : [newRestaurant];
      });
...
export const useUpdateRestaurant = () => {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, updates }) => {
      ...
      // Actualización optimista
      queryClient.setQueryData(["restaurants"], (old) => {
        return old ? old.map(r => r.id === id ? { ...r, ...updates } : r) : old;
      });
    ...
    onError: (error, variables, context) => {
      ...
      if (context?.previousRestaurant) {
        queryClient.setQueryData(["restaurant", variables.id], context.previousRestaurant);
      }
    },
    onSuccess: (data, variables) => {
      // Actualizar cache con datos reales
...
export const useDeleteRestaurant = () => {
  const queryClient = useQueryClient();
  const { handleError } = useErrorHandler();
  const { removeRestaurant } = useRestaurant();
