...
export const usePaymentStatus = (externalReference = null) => {
  ...
  useEffect(() => {
    ...
    if (externalReference) {
      // Si tenemos external_reference, escuchar solo ese pago (sin ordenamiento para evitar índices)
      q = query(
        paymentsRef,
      ...
      // Escuchar todos los pagos recientes del restaurante (sin ordenamiento)
      q = query(paymentsRef, where("restaurantId", "==", restaurant.id));
    }
    ...
    const unsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        try {
          if (!snapshot.empty) {
            // Si tenemos external_reference, buscar el documento específico
            let latestPayment;
            if (externalReference) {
              latestPayment = snapshot.docs
                .find(
                  (doc) => doc.data().externalReference === externalReference
                )
                ?.data();
            } else {
              // Si no hay external_reference, tomar el más reciente
            ...
            if (latestPayment) {
              ...
              if (latestPayment.status === "approved") {
                ...
                try {
                  ...
                  console.warn(
                    "⚠️ No se pudo liberar la mesa localmente:",
                    error
                  );
                }
      ...
      (err) => {
        ...
        if (err.message && err.message.includes("requires an index")) {
          console.warn(
            "⚠️ Índice de Firestore faltante. Creando consulta alternativa..."
          );
