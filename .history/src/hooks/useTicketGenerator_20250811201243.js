...
export const useTicketGenerator = () => {
  ...
  const generateTicket = async (orderData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Error al generar el ticket: ${response.status} ${response.statusText}`);
      }
      ...
      const errorMessage = err.message || "Error desconocido al generar el ticket";
  ...
  const printTicket = async (orderData) => {
    try {
      ...
      const errorMessage = err.message || "Error desconocido al imprimir el ticket";
      setError(errorMessage);
  ...
  const downloadTicket = async (orderData) => {
    try {
      ...
      const errorMessage = err.message || "Error desconocido al descargar el ticket";
      setError(errorMessage);
  ...
  const showTicketPreview = async (orderData) => {
    try {
      ...
      const errorMessage = err.message || "Error desconocido al generar la vista previa";
      setError(errorMessage);
