...
export const useInventarioOptimized = () => {
  ...
  const financialStats = useMemo(() => {
    ...
    return {
      ...stats,
      gananciaPorcentual: stats.costoStock > 0 ? (stats.ganancia / stats.costoStock * 100) : 0,
    };
  ...
  const lowStockItems = useMemo(() => {
    const allItems = [...bebidas, ...materiaPrima];
    return allItems.filter(item => (item.stock || 0) <= 5 && (item.stock || 0) > 0);
  }, [bebidas, materiaPrima]);
  ...
  const searchItems = useCallback((query, type = null) => {
    if (!query.trim()) {
    ...
    const filtered = allItems.filter(item => {
      const matchesQuery = item.nombre.toLowerCase().includes(searchTerm) ||
  ...
  const updateItem = useCallback(async (itemId, updates, type) => {
    try {
      ...
      const endpoint = type === 'bebida' ? '/api/stock' : '/api/materia-prima';
  ...
  const deleteItem = useCallback(async (itemId, type) => {
    try {
  ...
  }, [bebidas.length, materiaPrima.length, lowStockItems.length, outOfStockItems.length, lastUpdate]);
