  ...
  const registrarInicioSesion = async () => {
    return preventMultipleCalls(async () => {
      try {
        ...
        // Filtrar sesión abierta
        let sesionAbierta = null;
        snapshot.forEach((docu) => {
          const data = docu.data();
          // Consideramos abierta si horaCierre es null o undefined
          if (data.horaCierre === null || data.horaCierre === undefined) {
            if (
              !sesionAbierta ||
              new Date(data.horaApertura) > new Date(sesionAbierta.horaApertura)
            ) {
              sesionAbierta = { id: docu.id, ...data };
            }
          }
        });
