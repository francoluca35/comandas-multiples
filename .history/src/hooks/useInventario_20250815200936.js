...
export const useInventario = () => {
  ...
  const fetchMateriaPrima = async () => {
    try {
      const restauranteId = getRestaurantId();
      const response = await fetch(`/api/materia-prima?restauranteId=${restauranteId}`);

      if (!response.ok) {
        throw new Error("Error al obtener la materia prima");
      }
  ...
  const syncBebidaWithMenu = async (bebidaData, isUpdate = false, existingBebida = null) => {
    try {
      const restauranteId = getRestaurantId();
      ...
      if (existingBebidaMenu) {
        ...
        if (createResponse.ok) {
          ...
          console.error(`❌ Error creando bebida "${bebidaData.nombre}" en menú`);
        }
      }
    } catch (error) {
  ...
  const updateMateriaPrima = async (materiaPrimaId, updateData) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Error al actualizar la materia prima");
      }
  ...
  const deleteMateriaPrima = async (materiaPrimaId) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Error al eliminar la materia prima");
      }
