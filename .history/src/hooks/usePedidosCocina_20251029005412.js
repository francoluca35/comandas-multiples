...
const usePedidosCocina = () => {
  ...
  const addNewPedido = (nuevoPedido) => {
    setPedidos(prev => {
      ...
      const existePedido = prev.some(pedido => pedido.id === nuevoPedido.id);
      if (existePedido) {
        console.log("⚠️ Pedido ya existe, no se duplicará:", nuevoPedido.id);
        return prev;
      }
      console.log("➕ Agregando nuevo pedido:", nuevoPedido.id);
