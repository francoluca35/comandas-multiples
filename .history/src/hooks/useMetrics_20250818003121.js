...
export const useMetrics = () => {
  ...
  const loadHistoricalMetrics = useCallback(async (hours = 24, limit = 100) => {
    try {
      ...
      const response = await fetch(`/api/metrics?hours=${hours}&limit=${limit}`);
  ...
  const getMetricsByPeriod = useCallback((period = "24h") => {
    const now = Date.now();
    let since;
  ...
  const getTrends = useCallback(() => {
    ...
    const calculateTrend = (recentData, olderData, key) => {
      const recentAvg = recentData.reduce((sum, item) => sum + (item.summary?.performance?.[key] || 0), 0) / recentData.length;
      const olderAvg = olderData.reduce((sum, item) => sum + (item.summary?.performance?.[key] || 0), 0) / olderData.length;
