...
export const usePagos = () => {
  ...
  const calculatePaymentInfo = (restaurantData) => {
    ...
    const formatDateSafely = (dateString) => {
      ...
      // Si es una string de fecha, intentar parsearla
      const date = new Date(dateString);
      return isNaN(date.getTime()) ? null : date;
    };

    // Procesar fechas de forma segura
    const fechaActivacionDate = formatDateSafely(fechaActivacion);
    const proximoPagoDate = formatDateSafely(proximoPago);
    const ultimoPagoDate = formatDateSafely(ultimoPago);

    // Si no hay fecha de activación, usar la fecha de creación del documento
    const fechaActivacionFinal = fechaActivacionDate || new Date();

    // Si no hay próximo pago, calcularlo basado en la fecha de activación
    let proximoPagoFinal = proximoPagoDate;
    if (!proximoPagoFinal) {
      proximoPagoFinal = new Date(fechaActivacionFinal);
      if (periodicidad === "mensual") {
        proximoPagoFinal.setMonth(proximoPagoFinal.getMonth() + 1);
      } else {
        proximoPagoFinal.setFullYear(proximoPagoFinal.getFullYear() + 1);
      }
    }

    // Calcular días restantes hasta el próximo pago
    const today = new Date();
    const diffTime = proximoPagoFinal - today;
    ...
    return {
      ...
      proximoPago: proximoPagoFinal,
      ultimoPago: ultimoPagoDate,
      diasRestantes,
      estadoPago,
      cuotasPagadas,
      cuotasTotales,
  ...
  const deleteRestaurant = useCallback(
    async (restaurantId) => {
      try {
        ...
        return { success: true, message: "Restaurante eliminado completamente" };
      } catch (err) {
  ...
  // Suspender restaurante por falta de pago
  const suspendRestaurant = useCallback(
    async (restaurantId) => {
      try {
        ...
        return { success: true, message: "Restaurante suspendido por falta de pago" };
      } catch (err) {
  ...
  // Habilitar restaurante nuevamente
  const enableRestaurant = useCallback(
    async (restaurantId) => {
      try {
        ...
        return { success: true, message: "Restaurante habilitado nuevamente" };
      } catch (err) {
