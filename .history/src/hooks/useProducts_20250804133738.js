...
export const useProducts = (restaurantId = "francomputer") => {
  ...
  const createProduct = async (productData) => {
    try {
      ...
      if (!mainCategoryId || !subCategoryId || !name || !price) {
        throw new Error("Categoría principal, subcategoría, nombre y precio son obligatorios");
      }
  ...
  // Actualizar producto
  const updateProduct = async (productId, mainCategoryId, subCategoryId, productData) => {
    try {
      setLoading(true);
      setError(null);
