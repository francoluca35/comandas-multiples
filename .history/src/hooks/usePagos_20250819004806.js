...
export const usePagos = () => {
  ...
  const sendPaymentReminder = useCallback(async (restaurantId) => {
    try {
  ...
  const markPaymentAsPaid = useCallback(async (restaurantId) => {
    try {
  ...
  const getPaymentStats = useCallback(() => {
    ...
    const ingresosMensuales = restaurants.reduce((sum, r) => sum + (r.precio || 0), 0);

    return { total, alDia, vencidos, proximos, ingresosMensuales };
  }, [restaurants]);

  // Filtrar restaurantes
  const filterRestaurants = useCallback((searchTerm = "", filterStatus = "all") => {
