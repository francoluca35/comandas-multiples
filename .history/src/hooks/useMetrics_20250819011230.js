...
export const useMetrics = () => {
  ...
  const getRestaurantMetrics = useCallback(async (restaurantId) => {
    try {
      ...
      console.error(`❌ Error obteniendo métricas del restaurante ${restaurantId}:`, err);
      return null;
    }
  }, []);
  ...
  const getGeneralStats = useCallback(() => {
    ...
    const stats = restaurants.reduce((acc, restaurantId) => {
      ...
      return {
        ...
        totalRevenue: acc.totalRevenue + (orders.averageValue || 0) * (orders.total || 0),
    ...
    return {
      totalRestaurants,
      ...stats,
      averagePerformance: totalRestaurants > 0 ? stats.totalOrders / totalRestaurants : 0,
    };
  }, [metrics]);
