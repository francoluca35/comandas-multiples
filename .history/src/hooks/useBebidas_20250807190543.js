...
export const useBebidas = () => {
  ...
  const fetchBebidas = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/bebidas?restauranteId=${restauranteId}`);
  ...
  const updateHabilitacionByStock = async () => {
    const bebidasSinStock = bebidas.filter(bebida => bebida.stock === 0 && bebida.habilitada);
    const bebidasConStock = bebidas.filter(bebida => bebida.stock > 0 && !bebida.habilitada);
