...
const useKitchenNotifications = () => {
  ...
  const saveConfiguration = async (config = {}) => {
    ...
    try {
      // Crear objeto de configuración filtrando valores undefined
      const configToSave = {
      ...
      // Solo agregar campos de archivo personalizado si tienen valores válidos
      if (config.customAudioData !== undefined) {
        configToSave.customAudioData = config.customAudioData;
      } else if (customAudioData !== undefined) {
        configToSave.customAudioData = customAudioData;
      }

      if (config.customAudioName !== undefined) {
        configToSave.customAudioName = config.customAudioName;
      } else if (customAudioFile?.name !== undefined) {
        configToSave.customAudioName = customAudioFile.name;
      }

      if (config.customAudioMimeType !== undefined) {
        configToSave.customAudioMimeType = config.customAudioMimeType;
      } else if (customAudioFile?.type !== undefined) {
        configToSave.customAudioMimeType = customAudioFile.type;
      }
  ...
  // Helper para crear el objeto de configuración para guardar
  const createConfigObject = (updates) => {
    const configToSave = {
      isEnabled: updates.isEnabled !== undefined ? updates.isEnabled : isEnabled,
      volume: updates.volume !== undefined ? updates.volume : volume,
      soundType: updates.soundType !== undefined ? updates.soundType : soundType,
      updatedAt: new Date(),
    };

    if (updates.customAudioData !== undefined) {
      configToSave.customAudioData = updates.customAudioData;
    } else if (customAudioData !== undefined) {
      configToSave.customAudioData = customAudioData;
    }

    if (updates.customAudioName !== undefined) {
      configToSave.customAudioName = updates.customAudioName;
    } else if (customAudioFile?.name !== undefined) {
      configToSave.customAudioName = customAudioFile.name;
    }

    if (updates.customAudioMimeType !== undefined) {
      configToSave.customAudioMimeType = updates.customAudioMimeType;
    } else if (customAudioFile?.type !== undefined) {
      configToSave.customAudioMimeType = customAudioFile.type;
    }
  ...
  return {
    ...
    setIsEnabled: useCallback(async (enabled) => {
      ...
      if (restaurant?.id) {
        try {
          const configToSave = createConfigObject({ isEnabled: enabled });
          await setDoc(doc(db, "restaurantes", restaurant.id, "configuracion", "notificaciones"), configToSave);
    ...
    }, [restaurant?.id, createConfigObject]),
    
    setVolume: useCallback(async (newVolume) => {
      setVolume(newVolume);
      if (restaurant?.id) {
        try {
          const configToSave = createConfigObject({ volume: newVolume });
          await setDoc(doc(db, "restaurantes", restaurant.id, "configuracion", "notificaciones"), configToSave);
    ...
    setSoundType: useCallback(async (newSoundType) => {
      ...
      if (restaurant?.id) {
        try {
          const configToSave = createConfigObject({ soundType: newSoundType });
          await setDoc(doc(db, "restaurantes", restaurant.id, "configuracion", "notificaciones"), configToSave);
