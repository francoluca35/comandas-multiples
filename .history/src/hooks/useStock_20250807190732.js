...
export const useStock = () => {
  ...
  const syncWithBebidas = async (productoData, isUpdate = false, existingProducto = null) => {
    if (productoData.tipo === "bebida") {
      try {
        ...
        const existingBebida = bebidas.find(b => b.nombre.toLowerCase() === productoData.nombre.toLowerCase());
    ...
    try {
      ...
      if (productoToDelete?.tipo === "bebida") {
        try {
          const bebidasResponse = await fetch(`/api/bebidas?restauranteId=${restauranteId}`);
          const bebidas = await bebidasResponse.json();
          const existingBebida = bebidas.find(b => b.nombre.toLowerCase() === productoToDelete.nombre.toLowerCase());
          
          if (existingBebida) {
            await fetch(`/api/bebidas?restauranteId=${restauranteId}&bebidaId=${existingBebida.id}`, {
              method: "DELETE",
            });
          }
        } catch (error) {
