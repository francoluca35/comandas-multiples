...
export const useCache = () => {
  ...
  const setCachedData = useCallback((key, data, config = CACHE_CONFIG.CRITICAL) => {
    try {
      const cacheData = {
        data,
        timestamp: Date.now(),
  ...
  const smartCache = useCallback(async (key, fetchFunction, config = CACHE_CONFIG.CRITICAL) => {
    ...
    if (isOnline) {
      try {
        ...
        if (expiredCache) {
          ...
          return { data: expiredCache, source: 'expired_cache', offline: true };
        }
  ...
  const getCacheStats = useCallback(() => {
    const totalItems = Object.keys(localStorage).filter(key => 
      key.includes('_') && key.split('_')[0] in CACHE_CONFIG
    ).length;
  ...
  const precache = useCallback(async (dataMap) => {
    const promises = Object.entries(dataMap).map(([key, { fetchFn, config }]) => 
