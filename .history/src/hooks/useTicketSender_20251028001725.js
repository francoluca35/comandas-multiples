...
export const useTicketSender = () => {
  ...
  const sendTicketWhatsApp = async (orderData, pdfBlob, whatsappNumber) => {
    ...
    try {
      ...
      console.log("🔍 Número de WhatsApp original:", whatsappNumber);
      ...
      if (!cleanNumber || cleanNumber.length < 8) {
        throw new Error(`Formato de número de WhatsApp inválido: ${whatsappNumber}. Debe tener al menos 8 dígitos.`);
      }
      ...
      if (cleanNumber.startsWith('54')) {
        ...
        throw new Error(`Formato de número de WhatsApp inválido: ${whatsappNumber}. Número limpio: ${cleanNumber}`);
      }
      ...
      // Formatear nombre del cliente para el archivo (sin espacios, caracteres especiales)
      ...
      // Nombre del archivo: ticket-pago-(nombre-cliente)-fecha
      const nombreArchivo = `ticket-pago-${nombreCliente}-${fecha}.pdf`;
