...
export const useTables = () => {
  ...
  const reorderTablesForLocation = async (targetNumber, targetLocation) => {
    ...
    for (let i = 0; i < remainingTables.length; i++) {
      ...
      const newNumber = String(lastNumberInTarget + tablesToMove + i + 1).padStart(2, "0");
  ...
  const reorderAllTablesAfterDelete = async () => {
    ...
    const allTablesSorted = [...tables].sort((a, b) => parseInt(a.numero) - parseInt(b.numero));
    
    // Reordenar todas las mesas para que tengan números secuenciales
  ...
  const moveTableToLocation = async (targetNumber, targetLocation) => {
    ...
    // Mover la mesa
    await updateTable(tableToMove.id, { numero: newNumber, lugar: targetLocation });
