...
export const orderSchema = z.object({
  ...
  status: z.enum(["pendiente", "preparando", "listo", "entregado", "cancelado"], {
    errorMap: () => ({ message: "El estado seleccionado no es válido" }),
  }).default("pendiente"),
  paymentMethod: z.enum(["efectivo", "tarjeta", "mercadopago"], {
...
export const inventoryMovementSchema = z.object({
  ...
  type: z.enum(["entrada", "salida", "ajuste"], {
    errorMap: () => ({ message: "El tipo de movimiento seleccionado no es válido" }),
  }),
  quantity: z
    .number()
...
export const userSettingsSchema = z.object({
  ...
  dateFormat: z.enum(["DD/MM/YYYY", "MM/DD/YYYY", "YYYY-MM-DD"], {
    errorMap: () => ({ message: "El formato de fecha seleccionado no es válido" }),
  }).default("DD/MM/YYYY"),
  timeFormat: z.enum(["12h", "24h"], {
    errorMap: () => ({ message: "El formato de hora seleccionado no es válido" }),
  }).default("24h"),
  ...
  security: z.object({
    ...
    passwordPolicy: z.enum(["weak", "medium", "strong"], {
      errorMap: () => ({ message: "La política de contraseña seleccionada no es válida" }),
    }).default("strong"),
  }).default({}),
});
...
export const searchSchema = z.object({
  ...
  sortBy: z.enum(["name", "price", "date", "popularity"], {
    errorMap: () => ({ message: "El criterio de ordenamiento seleccionado no es válido" }),
  }).default("name"),
