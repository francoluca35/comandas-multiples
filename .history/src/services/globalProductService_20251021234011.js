...
class GlobalProductService {
    ...
    this.apis = [
      {
        ...
        priority: 1,
        useProxy: false // OpenFoodFacts permite CORS
      },
      {
        name: 'UPCItemDB',
        baseUrl: 'https://api.upcitemdb.com/prod/trial/lookup',
        priority: 2,
        useProxy: false // UPCItemDB permite CORS
      },
      {
        name: 'MockAPI',
  ...
  async searchInAPI(api, barcode) {
    switch (api.name) {
      case 'ProductOpenData':
        return await this.searchProductOpenData(barcode);
      case 'OpenFoodFacts':
  ...
  async searchOpenFoodFacts(barcode) {
    try {
      ...
      // Usar proxy CORS para evitar problemas de CORS
      const proxyUrl = `${this.apis[0].corsProxy}${encodeURIComponent(`${this.apis[0].baseUrl}/${barcode}.json`)}`;
  ...
  async searchUPCItemDB(barcode) {
    try {
      ...
      // Usar proxy CORS para evitar problemas de CORS
      const proxyUrl = `${this.apis[1].corsProxy}${encodeURIComponent(`${this.apis[1].baseUrl}?upc=${barcode}`)}`;
