...
class GlobalProductService {
  ...
  async searchLocalDatabase(barcode) {
    try {
      ...
      // Base de datos local de productos conocidos (Argentina y internacionales)
      const localProducts = {
        // Productos internacionales conocidos
        '3017620422003': {
          nombre: 'Nutella',
          marca: 'Ferrero',
        ...
        '7790895000002': {
          ...
          marca: 'Bimbo',
          categoria: 'Panadería',
          descripcion: 'Pan de molde lactal',
          precio: 0,
          stock: 0,
          proveedor: 'Bimbo',
          costo: 0,
        ...
        '7790895000009': {
          ...
          marca: 'La Española',
          categoria: 'Conservas',
          descripcion: 'Aceitunas verdes sin carozo',
          precio: 0,
          stock: 0,
  ...
  async searchOpenFoodFacts(barcode) {
    try {
      ...
      const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(`${this.apis[1].baseUrl}/${barcode}.json`)}`;
  ...
  async searchUPCItemDB(barcode) {
    try {
      ...
      // Usar proxy CORS para evitar problemas
      const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(`${this.apis[2].baseUrl}?upc=${barcode}`)}`;
  ...
  generateSmartProductInfo(barcode, countryCode, manufacturerCode) {
    ...
    // Mapeo de códigos de país (incluyendo Argentina)
    const countryMap = {
      '301': 'Francia',
      '400': 'Alemania', 
      '500': 'Reino Unido',
      ...
      '750': 'México',
      '760': 'Suiza',
      '779': 'Argentina', // Código principal de Argentina
      '780': 'Chile',
      '784': 'Paraguay',
      '786': 'Ecuador',
      '789': 'Brasil',
    ...
    // Generar nombre inteligente basado en el país
    let productNames = [];
    let brandNames = [];
    ...
    return {
      ...
      marca: randomBrand,
      categoria: category,
      descripcion: `Producto de ${category.toLowerCase()} ${country === 'Argentina' ? 'argentino' : 'internacional'} con código de barras ${barcode}`,
      imagen: '',
      codigoBarras: barcode,
      precio: 0,
      stock: 0,
