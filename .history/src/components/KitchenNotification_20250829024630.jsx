    ...
    <div className="fixed top-4 right-4 z-50 max-w-md w-full">
      ...
      <div className="mt-3 bg-slate-800 border border-slate-600 rounded-lg p-3 shadow-lg">
        ...
        <div className="space-y-3">
          ...
          {/* Área de Prueba de Sonidos */}
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-slate-300 text-sm font-medium">🎵 Prueba de Sonidos</span>
              <button
                onClick={onTestSound}
            ...
            {/* Grid de botones de prueba */}
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => onTestSpecificSound("powerful")}
              ...
              <button
                onClick={() => onTestSpecificSound("simple")}
                className={`p-2 rounded-lg text-xs font-medium transition-all duration-200 ${
                  soundType === "simple" 
                    ? "bg-blue-600 text-white shadow-lg" 
                    : "bg-slate-700 text-slate-300 hover:bg-slate-600"
              ...
              <button
                onClick={() => onTestSpecificSound("alarm")}
                className={`p-2 rounded-lg text-xs font-medium transition-all duration-200 ${
                  soundType === "alarm" 
                    ? "bg-blue-600 text-white shadow-lg" 
                    : "bg-slate-700 text-slate-300 hover:bg-slate-600"
              ...
              <button
                onClick={() => onTestSpecificSound("kitchen")}
                className={`p-2 rounded-lg text-xs font-medium transition-all duration-200 ${
                  soundType === "kitchen" 
                    ? "bg-blue-600 text-white shadow-lg" 
                    : "bg-slate-700 text-slate-300 hover:bg-slate-600"
              ...
              <button
                onClick={() => onTestSpecificSound("custom")}
          ...
          {/* Tipo de sonido seleccionado */}
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-slate-300 text-sm">Sonido Seleccionado</span>
            </div>
            <select
              value={soundType}
            ...
            >
              ...
              <option value="alarm">�� WhatsApp</option>
              <option value="kitchen">🍳 Cocina Tradicional</option>
              <option value="custom" disabled={!customAudioFile}>
                📁 Archivo Personalizado {customAudioFile ? `(${customAudioFile.name})` : '(No seleccionado)'}
              </option>
            </select>
          </div>
