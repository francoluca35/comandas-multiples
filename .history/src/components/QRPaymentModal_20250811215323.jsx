...
function QRPaymentModal({ isOpen, onClose, paymentData, orderData }) {
  ...
  useEffect(() => {
    console.log("QRPaymentModal useEffect triggered:", { isOpen, paymentData });
    if (isOpen && paymentData?.initPoint) {
      console.log("Generating QR code with initPoint:", paymentData.initPoint);
      generateQRCode();
    } else {
      ...
      <div className="bg-white rounded-lg p-6 w-96 max-w-[90vw] max-h-[90vh] overflow-y-auto">
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          ...
          <ol className="text-sm text-blue-700 space-y-1">
            ...
            <li>2. <strong>Importante:</strong> Asegúrate de usar una cuenta diferente a la del restaurante</li>
          ...
          <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-start space-x-2">
              ...
              <div className="text-yellow-800 text-sm">
                <p className="font-medium">¿Error "No podés pagarte a vos mismo"?</p>
                <p className="text-xs mt-1">Esto ocurre cuando intentas pagar con la misma cuenta del restaurante. Usa una cuenta diferente o contacta al administrador.</p>
              </div>
            </div>
          </div>
        </div>
