...
export const RestaurantGuard = ({ children }) => {
  ...
  useEffect(() => {
    console.log("🔍 RestaurantGuard - Estado:", {
      loading,
      restauranteActual: !!restauranteActual,
    });

    if (!loading && !restauranteActual) {
      console.log(
        "⚠️ RestaurantGuard - Redirigiendo al login: No hay restaurante cargado"
      );
      // Si no hay restaurante cargado, redirigir al login de comandas
      router.push("/home-comandas");
    } else if (!loading && restauranteActual) {
      console.log(
        "✅ RestaurantGuard - Restaurante cargado correctamente:",
