...
const BarcodeScanner = ({ 
  ...
  const { safeDOMOperation, elementExists, cleanup } = useSafeDOM();
  ...
  useEffect(() => {
    ...
    return () => {
      // Solo limpiar si el componente está montado y el scanner está activo
  ...
  const stopScanning = () => {
    if (html5QrcodeScannerRef.current) {
      safeDOMOperation(() => {
        // Verificar que el elemento DOM existe antes de limpiar
        if (elementExists('barcode-scanner')) {
