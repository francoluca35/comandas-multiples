...
const LoadingSpinner = React.memo(
  ({ size = "md", variant = "primary", text = "", className = "" }) => {
    const sizeClasses = {
      xs: "w-3 h-3",
      sm: "w-4 h-4",
      md: "w-6 h-6",
...
const Skeleton = React.memo(
  ({ variant = "text", lines = 1, className = "" }) => {
...
const LoadingOverlay = React.memo(
  ({ isLoading, children, text = "Cargando...", backdrop = true }) => {
...
const LoadingInline = React.memo(({ isLoading, children, fallback = null }) => {
  if (isLoading) {
...
const LoadingDots = React.memo(({ text = "Cargando", className = "" }) => {
  return (
...
const LoadingPulse = React.memo(({ size = "md", className = "" }) => {
  const sizeClasses = {
    sm: "w-4 h-4",
    md: "w-6 h-6",
    lg: "w-8 h-8",
...
const LoadingList = React.memo(({ items = 5, className = "" }) => {
  return (
...
const LoadingCards = React.memo(({ cards = 3, className = "" }) => {
  return (
    <div
...
const LoadingTable = React.memo(({ rows = 5, columns = 4, className = "" }) => {
  return (
    <div
      className={`overflow-hidden border border-slate-700/50 rounded-xl ${className}`}
    >
