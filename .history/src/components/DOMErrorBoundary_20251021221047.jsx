"use client";
import React from 'react';

/**
 * Error Boundary específico para errores de DOM
 * Captura errores de removeChild y otros errores de manipulación DOM
 */
class DOMErrorBoundary extends React.Component {
  constructor(props) {
    super(props);
    this.state = { 
      hasError: false, 
      error: null, 
      errorInfo: null,
      retryCount: 0 
    };
  }

  static getDerivedStateFromError(error) {
    // Verificar si es un error de DOM
    const isDOMError = error.name === 'NotFoundError' && 
                      error.message.includes('removeChild');
    
    if (isDOMError) {
      console.warn('DOM Error caught by boundary:', error);
      return { hasError: true, error, isDOMError: true };
    }
    
    return { hasError: true, error, isDOMError: false };
  }

  componentDidCatch(error, errorInfo) {
    this.setState({
      error,
      errorInfo
    });

    // Log del error para debugging
    console.error('DOM Error Boundary caught an error:', error, errorInfo);
    
    // Si es un error de DOM, intentar limpiar el estado
    if (error.name === 'NotFoundError' && error.message.includes('removeChild')) {
      this.handleDOMError();
    }
  }

  handleDOMError = () => {
    // Limpiar cualquier estado problemático
    try {
      // Forzar re-render del componente
      this.setState({ retryCount: this.state.retryCount + 1 });
    } catch (cleanupError) {
      console.warn('Error during DOM cleanup:', cleanupError);
    }
  };

  handleRetry = () => {
    this.setState({ 
      hasError: false, 
      error: null, 
      errorInfo: null,
      retryCount: this.state.retryCount + 1 
    });
  };

  render() {
    if (this.state.hasError) {
      // Si es un error de DOM, mostrar un mensaje más específico
      if (this.state.isDOMError) {
        return (
          <div className="min-h-screen flex items-center justify-center bg-gray-100">
            <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6">
              <div className="flex items-center mb-4">
                <div className="flex-shrink-0">
                  <svg className="h-8 w-8 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
                  </svg>
                </div>
                <div className="ml-3">
                  <h3 className="text-lg font-medium text-gray-900">
                    Error de Interfaz
                  </h3>
                </div>
              </div>
              
              <div className="mt-2">
                <p className="text-sm text-gray-600">
                  Se produjo un error temporal en la interfaz. Esto puede ocurrir cuando se cierran ventanas muy rápidamente.
                </p>
              </div>
              
              <div className="mt-4 flex space-x-3">
                <button
                  onClick={this.handleRetry}
                  className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  Reintentar
                </button>
                <button
                  onClick={() => window.location.reload()}
                  className="inline-flex items-center px-3 py-2 border border-gray-300 text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  Recargar Página
                </button>
              </div>
            </div>
          </div>
        );
      }

      // Para otros errores, mostrar el error boundary estándar
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
          <div className="max-w-md w-full bg-white shadow-lg rounded-lg p-6">
            <div className="flex items-center mb-4">
              <div className="flex-shrink-0">
                <svg className="h-8 w-8 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
              </div>
              <div className="ml-3">
                <h3 className="text-lg font-medium text-gray-900">
                  Error de Aplicación
                </h3>
              </div>
            </div>
            
            <div className="mt-2">
              <p className="text-sm text-gray-600">
                Se produjo un error inesperado. Por favor, recarga la página.
              </p>
            </div>
            
            <div className="mt-4">
              <button
                onClick={() => window.location.reload()}
                className="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
              >
                Recargar Página
              </button>
            </div>
          </div>
        </div>
      );
    }

    return this.props.children;
  }
}

export default DOMErrorBoundary;
