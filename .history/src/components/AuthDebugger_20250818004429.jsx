...
export default function AuthDebugger() {
  ...
  if (process.env.NODE_ENV !== "development" || !window.location.pathname.includes("/home-master")) {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-4 right-4 bg-black bg-opacity-90 text-white p-4 rounded-lg text-xs max-w-md z-50">
      ...
      <div className="space-y-1">
        ...
        <div><strong>Loading:</strong> {debugInfo.authContext?.loading ? "Yes" : "No"}</div>
        <div><strong>Has Usuario:</strong> {debugInfo.authContext?.hasUsuario ? "Yes" : "No"}</div>
        <div><strong>Is SuperAdmin:</strong> {debugInfo.authContext?.isSuperAdmin ? "Yes" : "No"}</div>
        <div><strong>Firebase User:</strong> {debugInfo.firebaseAuth?.hasCurrentUser ? "Yes" : "No"}</div>
        <div><strong>LocalStorage User:</strong> {debugInfo.localStorage?.hasSuperAdminUser ? "Yes" : "No"}</div>
        <div><strong>LocalStorage Role:</strong> {debugInfo.localStorage?.hasSuperAdminRole ? "Yes" : "No"}</div>
      </div>
    </div>
  );
}
