...
const BiometricSettings = ({ userId, username }) => {
  ...
  const loadCredentials = async () => {
    try {
      ...
      // Cargar credenciales desde IndexedDB (almacenamiento local)
      const localCredentials = await getLocalCredentials(userId);
      
      // También cargar desde Firestore para sincronización
      ...
      if (response.ok) {
        ...
        firestoreCredentials = data.credentials || [];
      }

      // Combinar credenciales locales y de Firestore
      // Priorizar las locales ya que son las que realmente funcionan
      const combinedCredentials = localCredentials.map(localCred => {
        const firestoreCred = firestoreCredentials.find(fc => fc.id === localCred.id);
        return {
          ...localCred,
          name: firestoreCred?.name || `Huella ${localCred.id}`,
          createdAt: firestoreCred?.createdAt || localCred.createdAt,
        };
      });
    ...
    <div className="bg-slate-800 rounded-lg p-6">
      ...
      <div className="bg-slate-700/50 rounded-lg p-4">
        ...
        <ul className="text-slate-400 text-sm space-y-1">
          ...
          <li>• Las huellas se mantienen guardadas incluso si cierras el navegador</li>
        </ul>
      </div>
