...
function POSPaymentModal({ isOpen, onClose, orderData, onPaymentComplete }) {
  ...
  useEffect(() => {
    if (isApproved && externalReference) {
      ...
      registrarIngresoAutomatico("tarjeta", orderData?.monto || 0);
      if (onPaymentComplete) {
        onPaymentComplete("tarjeta");
      }
      handleClose();
    }
  }, [isApproved, externalReference, onPaymentComplete, orderData]);

  // Función para registrar ingreso automáticamente
  const registrarIngresoAutomatico = async (metodoPago, monto) => {
    try {
      console.log("💰 Registrando ingreso automático (POS):", {
        metodoPago,
        monto,
      });
      ...
      console.log("✅ Ingreso registrado exitosamente (POS):", {
        tipoIngreso,
        motivo,
        monto,
        formaIngreso,
      ...
      console.error("❌ Error registrando ingreso automático (POS):", error);
  ...
  const confirmarPagoManual = async () => {
    try {
      ...
      if (result.success) {
        ...
        // Registrar ingreso automáticamente
        await registrarIngresoAutomatico("tarjeta", orderData?.monto || 0);
        if (onPaymentComplete) {
          onPaymentComplete("tarjeta");
        }
        handleClose();
        ...
        alert("Error al confirmar el pago: " + errorMessage);
      }
    } catch (error) {
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  const response = await fetch("/api/pagos/pos-externo/test", {
                    method: "POST",
                  ...
                  alert("Test completado. Revisa la consola.");
                } catch (error) {
                  console.error("🧪 Error en test:", error);
                  alert("Error en test: " + error.message);
                }
              }}
              className="w-full bg-green-500 text-white rounded-lg px-4 py-2 hover:bg-green-600 transition-colors text-sm"
            >
              🧪 Test Endpoint
            </button>
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  const result = await response.json();
                  console.log("🧪 Resultado del pago simple:", result);
                  alert("Pago simple completado. Revisa la consola.");
                } catch (error) {
                  console.error("🧪 Error en pago simple:", error);
                  alert("Error en pago simple: " + error.message);
                }
              }}
              className="w-full bg-purple-500 text-white rounded-lg px-4 py-2 hover:bg-purple-600 transition-colors text-sm"
            >
              🧪 Pago Simple
            </button>
          </div>
