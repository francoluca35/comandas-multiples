...
// Sistema de Métricas Globales para toda la aplicación
class MetricsCollector {
  ...
  start() {
    ...
    // Flush de métricas cada 30 segundos
  ...
  recordResourceLoad(entry) {
    ...
    if (entry.initiatorType === "fetch" || entry.name.includes("/api/")) {
      ...
      this.metrics.system.network.avgResponseTime = 
        (this.metrics.system.network.avgResponseTime * (this.metrics.system.network.requests - 1) + entry.duration) / 
        this.metrics.system.network.requests;
    }
  }
  ...
  recordApiCall(url, duration, status, error = null) {
    ...
    this.metrics.performance.apiCalls[url] = this.metrics.performance.apiCalls[url] || {
      count: 0,
      totalDuration: 0,
      avgDuration: 0,
      errors: 0,
  ...
  detectBusinessActions(url, response) {
    ...
    if (urlStr.includes("/api/stock") || urlStr.includes("/api/materia-prima")) {
  ...
  recordBusinessAction(action, data = {}) {
    ...
    if (this.metrics.performance.userActions.length > 100) {
      this.metrics.performance.userActions = this.metrics.performance.userActions.slice(-100);
    }
  }
  ...
  recordError(error, type = "unknown") {
    ...
    if (this.metrics.performance.errors.length > 50) {
      this.metrics.performance.errors = this.metrics.performance.errors.slice(-50);
    }
  }
  ...
  recordOrder(orderData) {
    ...
    const currentTotal = this.metrics.business.orders.averageValue * (this.metrics.business.orders.total - 1);
    this.metrics.business.orders.averageValue = (currentTotal + (orderData.total || 0)) / this.metrics.business.orders.total;
  }
  ...
  recordInventoryUpdate(inventoryData) {
    ...
    this.metrics.business.inventory.totalValue += (inventoryData.stock || 0) * (inventoryData.precio || 0);
    ...
    if (this.metrics.business.inventory.movements.length > 100) {
      this.metrics.business.inventory.movements = this.metrics.business.inventory.movements.slice(-100);
    }
  }
  ...
  recordUserSession(userData) {
    ...
    if (this.metrics.business.users.sessions.length > 100) {
      this.metrics.business.users.sessions = this.metrics.business.users.sessions.slice(-100);
    }
  }
  ...
  getSummary() {
    ...
    return {
      performance: {
        totalPageLoads: Object.values(metrics.performance.pageLoads).reduce((sum, page) => sum + page.count, 0),
        avgPageLoadTime: Object.values(metrics.performance.pageLoads).reduce((sum, page) => sum + page.avgTime, 0) / Math.max(Object.keys(metrics.performance.pageLoads).length, 1),
        cacheHitRate: metrics.performance.cacheHits / Math.max(metrics.performance.cacheHits + metrics.performance.cacheMisses, 1) * 100,
        totalErrors: metrics.performance.errors.length,
        totalApiCalls: Object.values(metrics.performance.apiCalls).reduce((sum, api) => sum + api.count, 0),
        avgApiResponseTime: Object.values(metrics.performance.apiCalls).reduce((sum, api) => sum + api.avgDuration, 0) / Math.max(Object.keys(metrics.performance.apiCalls).length, 1),
      },
      business: {
      ...
      system: {
        ...
        memoryUsage: metrics.system.memory.length > 0 ? metrics.system.memory[metrics.system.memory.length - 1] : null,
...
// Iniciar automáticamente en el cliente
