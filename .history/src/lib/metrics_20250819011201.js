...
class MetricsCollector {
  ...
  async getRestaurantData(restaurantId) {
    try {
      ...
      data.inventory = stockSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

      // Obtener mesas
      const tablesRef = collection(db, "restaurantes", restaurantId, "tables");
      const tablesSnapshot = await getDocs(tablesRef);
      data.tables = tablesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      ...
      console.error(`Error obteniendo datos del restaurante ${restaurantId}:`, error);
      return { orders: [], inventory: [], tables: [], users: [], apiCalls: 0, errors: [] };
    }
  }
  ...
  async generateBusinessMetrics(restaurantId, data) {
    ...
    metrics.orders.pending = data.orders.filter(order => order.estado === 'pendiente').length;
    metrics.orders.completed = data.orders.filter(order => order.estado === 'completado').length;
    ...
    metrics.inventory.lowStock = data.inventory.filter(item => item.cantidad <= (item.minimo || 10)).length;
    metrics.inventory.outOfStock = data.inventory.filter(item => item.cantidad <= 0).length;
    metrics.inventory.totalValue = this.calculateInventoryValue(data.inventory);

    // Métricas de mesas
    metrics.tables.total = data.tables.length;
    metrics.tables.occupied = data.tables.filter(table => table.estado === 'ocupada').length;
    metrics.tables.free = data.tables.filter(table => table.estado === 'libre').length;
    metrics.tables.averageOccupancy = data.tables.length > 0 ? (metrics.tables.occupied / data.tables.length) * 100 : 0;
  ...
  calculateAverageResponseTime(orders) {
    ...
    const responseTimes = orders.map(order => order.responseTime || 0).filter(time => time > 0);
  ...
  async getRestaurantMetrics(restaurantId) {
    try {
      ...
      console.error(`❌ Error obteniendo métricas del restaurante ${restaurantId}:`, error);
      return null;
    }
  }
}
