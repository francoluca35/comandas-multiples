    ...
    for (const restauranteDoc of restaurantesSnap.docs) {
      ...
      for (const [userName, userInstances] of usuariosPorNombre) {
        if (userInstances.length > 1) {
          ...
          const keepInstance = userInstances.find(
            (instance) => instance.id === userName
          );
          const deleteInstances = userInstances.filter(
            (instance) => instance.id !== userName
          );
          ...
          if (keepInstance) {
            ...
            for (const deleteInstance of deleteInstances) {
              ...
              await deleteDoc(
                doc(
                  db,
                  `restaurantes/${restauranteId}/users/${deleteInstance.id}`
                )
              );
            }
          } else {
            ...
            for (let i = 1; i < userInstances.length; i++) {
              ...
              await deleteDoc(
                doc(
                  db,
                  `restaurantes/${restauranteId}/users/${userInstances[i].id}`
                )
              );
            }
          }
