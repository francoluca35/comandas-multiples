...
const { getFirestore, collection, getDocs, deleteDoc, query, where, orderBy, limit } = require('firebase/firestore');
...
async function showMetricsStats() {
  try {
    ...
    console.log(`   • Total de restaurantes con métricas: ${restaurants.length}`);
    ...
    if (restaurants.length > 0) {
      ...
      restaurants.forEach((restaurant, index) => {
        ...
        console.log(`      - Ingresos promedio: $${restaurant.revenue.toFixed(2)}`);
...
async function deleteMetricsCollection(force = false, dryRun = false) {
  try {
    ...
    if (dryRun) {
      console.log('🔍 MODO DRY-RUN: No se eliminarán archivos, solo simulación');
      console.log(`📊 Se simularía la eliminación de ${stats.count} documentos`);
      return;
    }
