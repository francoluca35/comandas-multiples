...
// Función para verificar el estado de renderizado de componentes
const checkComponentRendering = () => {
  console.log("🎨 Verificando estado de renderizado de componentes...");
  
  // Verificar si estamos en la página home
  const currentPath = window.location.pathname;
  if (currentPath !== "/home-comandas/home") {
    console.log("❌ No estamos en la página home");
    return;
  }
  
  // Verificar elementos específicos del dashboard
  const turnoCard = document.querySelector('[class*="Turno"]');
  const ventasCard = document.querySelector('[class*="Ventas"]');
  const dineroCard = document.querySelector('[class*="Dinero"]');
  const stockCard = document.querySelector('[class*="Stock"]');
  const dispositivosCard = document.querySelector('[class*="Dispositivos"]');
  
  console.log("🔍 Elementos del dashboard detectados:");
  console.log("  - TurnoCard:", !!turnoCard);
  console.log("  - VentasCard:", !!ventasCard);
  console.log("  - DineroCard:", !!dineroCard);
  console.log("  - StockCard:", !!stockCard);
  console.log("  - DispositivosCard:", !!dispositivosCard);
  ...
  if (isAdmin) {
    const hasAllCards = turnoCard && ventasCard && dineroCard && stockCard && dispositivosCard;
    console.log("✅ ¿Tiene todas las tarjetas (admin)?", hasAllCards);
    
    if (!hasAllCards) {
      console.log("⚠️ Faltan tarjetas para admin, forzando actualización...");
      forceImmediateUpdate();
    }
  } else {
    const hasLimitedCards = turnoCard && ventasCard;
    console.log("✅ ¿Tiene tarjetas limitadas (no admin)?", hasLimitedCards);
    
    if (!hasLimitedCards) {
      console.log("⚠️ Faltan tarjetas para usuario no admin, forzando actualización...");
      forceImmediateUpdate();
    }
  }
};
...
const debugContextUpdate = () => {
  ...
  // 4. Verificar renderizado de componentes
  checkComponentRendering();
  
  // 5. Si hay datos de autenticación pero no hay turno, simular login
  ...
  console.log("- window.debugContextUpdate.checkComponentRendering() - Verificar renderizado de componentes");
...
console.log("5. Si hay problemas de renderizado, ejecuta: window.debugContextUpdate.checkComponentRendering()");
console.log("6. Si nada funciona, ejecuta: window.debugContextUpdate.smartReload()");
